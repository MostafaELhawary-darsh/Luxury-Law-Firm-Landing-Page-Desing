...
export default function Navbar({ onContactClick, onLibraryClick, onFinanceClick, onFirmClick, onEditorClick }: NavbarProps) {
  ...
  const navLinks = [
    ...
    { label: 'مكتبة المحرر', action: 'editor' as const },
              ...
              <button
                onClick={() => {
                  if ('action' in link && link.action === 'library') {
                    ...
                    onFirmClick();
                  } else if ('action' in link && link.action === 'editor') {
                    onEditorClick();
