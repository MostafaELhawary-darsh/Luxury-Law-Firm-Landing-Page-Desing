...
import {
  ...
  Activity, Server, AlertCircle, BadgeCheck, Stethoscope, DollarSign,
  FileText, Scale, Gavel, Users, HeartPulse, ShieldCheck,
} from 'lucide-react';
import { supabase, formatDate, formatCurrency } from '@/lib/financeUtils';
import type {
  M65MedicalFile, M65AuditLog,
} from '@/lib/firmTypes';
...
const STAGE_CONFIG: Record<string, { label: string; bg: string; text: string }> = {
  ...
  inspected: { label: 'تفتيش', bg: 'bg-cyan-50', text: 'text-cyan-700' },
...
const STAGES = ['draft', 'reviewed', 'inspected', 'approved', 'executed', 'terminated'];

const FILE_TYPE_LABELS: Record<string, string> = {
  licensing: 'ترخيص',
  malpractice: 'مسؤولية طبية',
  staff_contract: 'عقد كادر طبي',
  insurance: 'تأمين صحي',
};

const FILE_TYPE_ICONS: Record<string, typeof Stethoscope> = {
  licensing: FileText,
  malpractice: AlertCircle,
...
const emptyForm: MedicalForm = {
  ...
  facility_name: '', license_number: '', gahar_accredited: false,
  malpractice_status: 'none', insurance_claim_ref: '', claim_amount: '0',
  description: '',
};

export default function MedicalInstitutionsEngine({ voiceAdd }: { voiceAdd?: () => PendingAddCommand | null }) {
  const [files, setFiles] = useState<M65MedicalFile[]>([]);
  const [attorneys, setAttorneys] = useState<Attorney[]>([]);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<Tab>('files');
  const [selectedFile, setSelectedFile] = useState<M65MedicalFile | null>(null);
  const [auditLogs, setAuditLogs] = useState<M65AuditLog[]>([]);
  const [allAudit, setAllAudit] = useState<M65AuditLog[]>([]);
  const [detailLoading, setDetailLoading] = useState(false);
  const [modalOpen, setModalOpen] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [form, setForm] = useState<MedicalForm>(emptyForm);
  ...
  const fetchAll = useCallback(async () => {
    ...
    const [fRes, attRes, auditRes] = await Promise.all([
      supabase.from('m65_medical_files')
        .select('*, advisor:lf_attorneys(name)')
        .order('created_at', { ascending: false }),
      supabase.from('lf_attorneys').select('*').order('name'),
      supabase.from('m65_audit_logs').select('*').order('created_at', { ascending: false }).limit(50),
    ]);
    if (fRes.error) console.error('m65 fetch error', fRes.error);
    ...
    setFiles((fRes.data as M65MedicalFile[]) || []);
    setAttorneys((attRes.data as Attorney[]) || []);
    setAllAudit((auditRes.data as M65AuditLog[]) || []);
    setLoading(false);
  }, []);
  ...
  const logAudit = async (fileId: string, action: string, detail: string) => {
    ...
    const { error } = await supabase.from('m65_audit_logs').insert({
  ...
  const openEdit = (f: M65MedicalFile) => {
    setForm({
      file_number: f.file_number, file_title: f.file_title,
      file_type: f.file_type, stage: f.stage,
      facility_name: f.facility_name, license_number: f.license_number || '',
      gahar_accredited: f.gahar_accredited || false,
      malpractice_status: f.malpractice_status || 'none',
      insurance_claim_ref: f.insurance_claim_ref || '', claim_amount: String(f.claim_amount || 0),
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    const amount = Number(form.claim_amount) || 0;
    const payload = {
      ...
      gahar_accredited: form.gahar_accredited,
      malpractice_status: form.malpractice_status,
      insurance_claim_ref: form.insurance_claim_ref.trim() || null,
      claim_amount: amount,
      description: form.description.trim() || null,
    };
    if (editingId) {
      const { error } = await supabase.from('m65_medical_files').update(payload).eq('id', editingId);
      if (error) console.error('update error', error);
      await logAudit(editingId, 'file_updated', 'تحديث بيانات الملف الطبي');
    } else {
      const { data, error } = await supabase.from('m65_medical_files').insert(payload).select('id');
      if (error) console.error('insert error', error);
      const newId = data?.[0]?.id;
      if (newId) {
        await logAudit(newId, 'file_created', 'إنشاء ملف طبي — النوع: ' + (FILE_TYPE_LABELS[form.file_type] || form.file_type));
        const safetyLinked = form.file_type === 'malpractice' || form.file_type === 'licensing';
        await supabase.from('m65_medical_files').update({
          m53_document_id: 'DOC-M65-' + Date.now().toString().slice(-6),
          m54_finance_linked: true,
          m10_case_opened: true,
          m91_safety_linked: safetyLinked,
          m109_biometric_signed: true,
          m92_notified: true,
          cost_center_id: 'CC-M65-' + Date.now().toString().slice(-6),
        }).eq('id', newId);
        ...
        if (safetyLinked) await logAudit(newId, 'm91_safety', 'ربط الملف بمحرك الأمن والسلامة (M91)');
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    const { error } = await supabase.from('m65_medical_files').delete().eq('id', deleteId);
  ...
  const openFileDetail = async (f: M65MedicalFile) => {
    setSelectedFile(f);
    setDetailLoading(true);
    const aRes = await supabase.from('m65_audit_logs').select('*').eq('case_id', f.id).order('created_at', { ascending: true });
    if (aRes.error) console.error('detail audit error', aRes.error);
    setAuditLogs((aRes.data as M65AuditLog[]) || []);
    setDetailLoading(false);
  };

  const advanceStage = async (f: M65MedicalFile) => {
    const idx = STAGES.indexOf(f.stage);
    if (idx < 0 || idx >= STAGES.length - 1) return;
    const next = STAGES[idx + 1];
    const { error } = await supabase.from('m65_medical_files').update({ stage: next, status: next === 'terminated' ? 'terminated' : 'active' }).eq('id', f.id);
    ...
    fetchAll();
    setSelectedFile({ ...f, stage: next } as M65MedicalFile);
  };
  ...
  const filteredFiles = files.filter((f) => {
    ...
    if (searchQuery) {
      ...
      if (!f.file_number.toLowerCase().includes(q) && !f.file_title.toLowerCase().includes(q) && !f.facility_name.toLowerCase().includes(q)) return false;
    }
    return true;
  });
  ...
  const activeMalpractice = files.filter((f) => f.malpractice_status !== 'none' && f.malpractice_status !== 'ruling').length;
  const gaharCount = files.filter((f) => f.gahar_accredited).length;
  const totalClaims = files.reduce((s, f) => s + (f.claim_amount || 0), 0);

  const tabs: { id: Tab; label: string; icon: typeof Stethoscope; badge?: number }[] = [
    { id: 'files', label: 'الملفات الطبية', icon: Stethoscope, badge: files.length },
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-heading font-bold text-midnight text-lg">المؤسسات الطبية والقطاعات الصحية واللوائح العلاجية (M65)</h2>
            <p className="font-body text-[10px] text-ink/40">إدارة تراخيص المنشآت الطبية واعتماد GAHAR والمسؤولية الطبية والتأمين الصحي الشامل</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard icon={<Stethoscope size={14} className="text-midnight" />} label="إجمالي الملفات" value={String(files.length)} valueClass="text-midnight" />
        <StatCard icon={<AlertCircle size={14} className="text-red-600" />} label="قضايا مسؤولية طبية نشطة" value={String(activeMalpractice)} valueClass="text-red-700" />
        <StatCard icon={<BadgeCheck size={14} className="text-green-600" />} label="معتمد GAHAR" value={String(gaharCount)} valueClass="text-green-700" />
        <StatCard icon={<DollarSign size={14} className="text-gold" />} label="إجمالي مبالغ المطالبات" value={formatCurrency(totalClaims)} valueClass="text-gold" />
      </div>
      ...
      <div className="bg-midnight rounded-xl p-4 border border-gold/20">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="font-heading font-bold text-cream text-xs">دورة حياة الملف الطبي — 6 مراحل</span>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-2">
          {[
            ...
            { icon: DollarSign, label: 'المحرك المالي (M54)', desc: 'ربط المطالبات', color: 'text-gold' },
            { icon: Server, label: 'المحرك الموحد (M10)', desc: 'فتح القضية', color: 'text-blue-600' },
            { icon: ShieldCheck, label: 'الأمن والسلامة (M91)', desc: 'سلامة المنشأة', color: 'text-red-600' },
        ...
        <div className="space-y-2">
          {filteredFiles.length === 0 ? (
            <div className="text-center py-12 bg-gray-50 rounded-xl border border-gray-200">
              <Stethoscope size={28} className="text-ink/15 mx-auto mb-2" />
              <p className="font-body text-xs text-ink/30">لا توجد ملفات طبية مسجلة</p>
            </div>
          ) : (
            filteredFiles.map((f) => {
              const sCfg = STAGE_CONFIG[f.stage] || STAGE_CONFIG.draft;
              const stageIdx = STAGES.indexOf(f.stage);
              const TypeIcon = FILE_TYPE_ICONS[f.file_type] || Stethoscope;
              return (
                    ...
                    <div className="flex items-start gap-3 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {f.gahar_accredited && <span className="flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[9px] font-body bg-green-50 text-green-600"><BadgeCheck size={8} /> GAHAR</span>}
                          {f.malpractice_status !== 'none' && <span className="flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[9px] font-body bg-red-50 text-red-600"><AlertCircle size={8} /> مسؤولية طبية</span>}
                        </div>
                        <p className="font-body text-xs font-bold text-midnight mt-1 leading-snug line-clamp-2">{f.file_title}</p>
                        <div className="flex items-center gap-2 mt-2 flex-wrap">
                          <span className="font-body text-[9px] text-ink/40">المنشأة: {f.facility_name}</span>
                          {f.license_number && <span className="font-body text-[9px] text-ink/40">رقم الترخيص: {f.license_number}</span>}
                          {f.claim_amount > 0 && <span className="font-body text-[9px] text-gold font-bold">{formatCurrency(f.claim_amount)}</span>}
                          {f.malpractice_status !== 'none' && <span className="font-body text-[9px] text-red-600 font-bold">حالة: {MALPRACTICE_LABELS[f.malpractice_status] || f.malpractice_status}</span>}
                          ...
                          {f.m91_safety_linked && <span className="flex items-center gap-0.5 px-1 py-0.5 rounded text-[9px] font-body bg-red-50 text-red-600"><ShieldCheck size={8} /> M91</span>}
              ...
              {allAudit.map((log) => (
                <div key={log.id} className="flex items-start gap-3 px-4 py-3 hover:bg-gray-50 transition-colors">
                  <div className="w-7 h-7 rounded-lg bg-gray-100 flex items-center justify-center flex-shrink-0">
                    {log.action.includes('created') ? <Stethoscope size={12} className="text-blue-600" />
                      ...
                      : log.action.includes('m91') ? <ShieldCheck size={12} className="text-red-600" />
          ...
          <div className="relative bg-white w-full max-w-lg h-full overflow-y-auto shadow-2xl flex flex-col">
            <div className="sticky top-0 bg-white border-b border-gray-100 px-5 py-4 flex items-center justify-between z-10">
              <div className="flex items-center gap-2">
                ...
                <span className="font-heading font-bold text-midnight text-sm">ملف المنشأة الطبية</span>
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
                ...
                <div className="bg-gold/5 rounded-lg p-3 border border-gold/10">
                  <div className="flex items-center gap-1.5 mb-2">
                    <Stethoscope size={12} className="text-gold" />
                    <span className="font-body text-[10px] font-bold text-midnight">بيانات الملف</span>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div><span className="font-body text-[9px] text-ink/40">المنشأة الطبية</span><p className="font-body text-xs font-bold text-midnight">{selectedFile.facility_name}</p></div>
                    ...
                    <div><span className="font-body text-[9px] text-ink/40">اعتماد GAHAR</span><p className="font-body text-xs font-bold text-midnight">{selectedFile.gahar_accredited ? 'معتمد ✓' : 'غير معتمد'}</p></div>
                ...
                {/* Claim amount */}
                {selectedFile.claim_amount > 0 && (
                  <div className="bg-gold/5 rounded-lg p-3 border border-gold/10">
                    <DollarSign size={12} className="text-gold mb-1" />
                    <span className="font-body text-[9px] text-ink/40">مبلغ المطالبة</span>
                    <p className="font-body text-sm font-bold text-gold">{formatCurrency(selectedFile.claim_amount)}</p>
                    {selectedFile.insurance_claim_ref && <p className="font-body text-[10px] text-ink/50 mt-1">مرجع المطالبة: {selectedFile.insurance_claim_ref}</p>}
                  </div>
                )}

                {/* GAHAR flag */}
                <div className="flex items-center gap-2 flex-wrap">
                  <span className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-body font-bold ${selectedFile.gahar_accredited ? 'bg-green-50 text-green-600' : 'bg-gray-100 text-ink/30'}`}><BadgeCheck size={10} /> اعتماد GAHAR {selectedFile.gahar_accredited ? 'معتمد' : 'غير معتمد'}</span>
                </div>
                ...
                <div className="bg-red-50 rounded-lg p-3 border border-red-100">
                  <div className="flex items-center gap-1.5 mb-2">
                    ...
                    <span className="font-body text-[10px] font-bold text-red-700">حالة المسؤولية الطبية</span>
                  </div>
                  <p className="font-body text-xs font-bold text-midnight">{MALPRACTICE_LABELS[selectedFile.malpractice_status] || selectedFile.malpractice_status}</p>
                </div>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-body font-bold ${selectedFile.m91_safety_linked ? 'bg-red-50 text-red-600' : 'bg-gray-100 text-ink/30'}`}><ShieldCheck size={10} /> M91 {selectedFile.m91_safety_linked ? 'مربوط' : 'غير مربوط'}</span>
      ...
      <EntityModal open={modalOpen} title={editingId ? 'تعديل الملف الطبي' : 'ملف طبي جديد'} onClose={() => setModalOpen(false)} onSubmit={handleSave} loading={saving}>
        <div className="grid grid-cols-2 gap-4">
          <Field label="رقم الملف" required><TextInput value={form.file_number} onChange={(e) => setForm({ ...form, file_number: e.target.value })} placeholder="MED-2025-001" /></Field>
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="اسم المنشأة الطبية" required><TextInput value={form.facility_name} onChange={(e) => setForm({ ...form, facility_name: e.target.value })} /></Field>
          <Field label="رقم الترخيص"><TextInput value={form.license_number} onChange={(e) => setForm({ ...form, license_number: e.target.value })} placeholder="GAHAR-LIC-001" /></Field>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Field label="المرحلة">
          ...
          <Field label="حالة المسؤولية الطبية">
            <Select value={form.malpractice_status} onChange={(e) => setForm({ ...form, malpractice_status: e.target.value })}>
              {Object.entries(MALPRACTICE_LABELS).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
            </Select>
          </Field>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Field label="مرجع المطالبة التأمينية"><TextInput value={form.insurance_claim_ref} onChange={(e) => setForm({ ...form, insurance_claim_ref: e.target.value })} placeholder="INS-2025-001" /></Field>
        ...
        <Checkbox label="معتمد GAHAR (GAHAR Accredited)" checked={form.gahar_accredited} onChange={(v) => setForm({ ...form, gahar_accredited: v })} />
