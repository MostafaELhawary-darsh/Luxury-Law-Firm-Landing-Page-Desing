...
export default function VaultConnectorEngine({ voiceAdd }: { voiceAdd?: () => PendingAddCommand | null }) {
  ...
  const handleSavePull = async () => {
    ...
    if (editingPullId) {
      ...
      const edgeUrl = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/vault-connector/pull`;
      const res = await fetch(edgeUrl, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${import.meta.env.VITE_SUPABASE_ANON_KEY}` },
        body: JSON.stringify({
          provider_code: provider?.provider_code || 'GENERIC',
          pull_type: pullForm.pull_type,
          pull_title: pullForm.pull_title.trim(),
          source_url: pullForm.source_url.trim() || undefined,
          source_format: pullForm.source_format.trim() || undefined,
          entity_id_linked: pullForm.entity_id_linked.trim() || undefined,
          description: pullForm.description.trim() || undefined,
        }),
      });
      if (!res.ok) {
        const errBody = await res.json().catch(() => ({}));
        console.error('vault-connector pull error', errBody);
      }
    }
    setPullSaving(false);
