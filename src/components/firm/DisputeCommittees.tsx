...
import {
  ...
  TrendingUp, MessageSquare, Video,
} from 'lucide-react';
...
type Tab = 'cases' | 'deadlines' | 'precedents' | 'recommendations' | 'live' | 'audit';
...
export default function DisputeCommittees({ voiceAdd }: { voiceAdd?: () => PendingAddCommand | null }) {
  ...
  const tabs: { id: Tab; label: string; icon: typeof ClipboardList; badge?: number }[] = [
    ...
    { id: 'live', label: 'جلسة اللجنة المباشرة', icon: Video },
  ...
  return (
    <div className="space-y-5">
      ...
      {activeTab === 'live' && (
        <LiveSessionLauncher
          sessionLabel="جلسة اللجنة المباشرة"
          contextType="dispute"
          sessions={cases.map((c) => ({
            id: c.id,
            title: c.case_number || `تظلم ${c.id.slice(0, 6)}`,
            date: c.grievance_filing_date || null,
            status: c.stage || 'active',
            participantNames: [c.challenged_authority, c.committee_name, c.advisor?.name].filter(Boolean) as string[],
            documents: [],
          }))}
          onCreateSession={(ctxId) => {
            ...
            if (!c) return null;
            return {
              title: `لجنة تظلم: ${c.case_number || ctxId.slice(0, 6)}`,
              subtitle: c.case_title || 'جلسة استماع لجنة التظلمات',
              contextType: 'dispute',
              contextId: ctxId,
              participants: [
                { name: c.challenged_authority || 'المتظلم', role: 'متظلم' },
                { name: c.committee_name || 'رئيس اللجنة', role: 'رئيس لجنة' },
                { name: c.advisor?.name || 'المستشار', role: 'مستشار' },
              ],
              documents: [],
            };
          }}
