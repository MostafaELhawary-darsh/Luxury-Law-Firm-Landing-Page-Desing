...
import {
  ...
  TrendingUp, AlertOctagon, ScanLine, Server,
} from 'lucide-react';
import { supabase, formatDate, formatCurrency } from '@/lib/financeUtils';
import type {
  M02AdminCase, M02AdminParty, M02AdminDeadline, M02ConstitutionalReview,
  M02BiometricApproval, M02AdminAuditLog,
} from '@/lib/firmTypes';
...
type Tab = 'cases' | 'deadlines' | 'constitutional' | 'biometric' | 'audit';

const STAGE_CONFIG: Record<string, { label: string; bg: string; text: string }> = {
  intake: { label: 'الاستلام والتصنيف', bg: 'bg-blue-50', text: 'text-blue-700' },
  deadlines: { label: 'الضبط الزمني الآلي', bg: 'bg-amber-50', text: 'text-amber-700' },
  defense_generation: { label: 'توليد الدفوع', bg: 'bg-purple-50', text: 'text-purple-700' },
  authentication: { label: 'المصادقة والتوثيق', bg: 'bg-green-50', text: 'text-green-700' },
};

const STAGES = ['intake', 'deadlines', 'defense_generation', 'authentication'];

const CATEGORY_LABELS: Record<string, string> = {
  annulment: 'إلغاء قرار إداري',
  compensation: 'تعويض إداري',
  disciplinary: 'قرار تأديبي',
  admin_contract: 'عقد إداري',
  settlement: 'تسوية',
  constitutional: 'تفسير دستوري',
};

const SUBTYPE_LABELS: Record<string, string> = {
  employment_dispute: 'منازعة توظيف حكومي',
  public_works: 'أشغال عامة',
  disciplinary_decision: 'قرار تأديبي',
...
const REVIEW_TYPE_LABELS: Record<string, string> = {
  law_review: 'فحص قانون',
  regulation_review: 'فحص لائحة',
  decision_review: 'فحص قرار',
  constitutional_interpretation: 'تفسير دستوري',
};

const REVIEW_RESULT_CONFIG: Record<string, { label: string; bg: string; text: string }> = {
...
const BIOMETRIC_STATUS_CONFIG: Record<string, { label: string; bg: string; text: string }> = {
  pending: { label: 'بانتظار المصادقة', bg: 'bg-amber-50', text: 'text-amber-600' },
  approved: { label: 'تمت المصادقة', bg: 'bg-green-50', text: 'text-green-600' },
  rejected: { label: 'مرفوض', bg: 'bg-red-50', text: 'text-red-600' },
};

const BIOMETRIC_METHOD_LABELS: Record<string, string> = {
  fingerprint: 'بصمة الإصبع',
  iris_scan: 'مسح قزحية العين',
...
const emptyForm: CaseForm = {
  ...
  stage: 'intake', court: '', court_circuit: '', challenged_decision: '', challenged_decision_date: '',
  challenged_authority: '', filing_date: '', next_hearing_date: '', success_rate_estimate: '50',
  financial_value: '0', court_fees: '0', assigned_advisor_id: '', description: '',
};

export default function AdministrativeCourt({ voiceAdd }: { voiceAdd?: () => PendingAddCommand | null }) {
  const [cases, setCases] = useState<M02AdminCase[]>([]);
  const [attorneys, setAttorneys] = useState<Attorney[]>([]);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<Tab>('cases');
  const [selectedCase, setSelectedCase] = useState<M02AdminCase | null>(null);
  const [parties, setParties] = useState<M02AdminParty[]>([]);
  const [deadlines, setDeadlines] = useState<M02AdminDeadline[]>([]);
  const [reviews, setReviews] = useState<M02ConstitutionalReview[]>([]);
  const [biometrics, setBiometrics] = useState<M02BiometricApproval[]>([]);
  const [auditLogs, setAuditLogs] = useState<M02AdminAuditLog[]>([]);
  const [allDeadlines, setAllDeadlines] = useState<M02AdminDeadline[]>([]);
  const [allReviews, setAllReviews] = useState<M02ConstitutionalReview[]>([]);
  const [allBiometrics, setAllBiometrics] = useState<M02BiometricApproval[]>([]);
  const [allAudit, setAllAudit] = useState<M02AdminAuditLog[]>([]);
  ...
  const [deleteType, setDeleteType] = useState<'case' | 'party' | 'deadline' | 'review' | 'biometric'>('case');
  ...
  const [reviewForm, setReviewForm] = useState({ review_type: 'decision_review', subject_title: '', subject_ref: '', constitutional_principle: '', findings: '', precedent_refs: '' });
  const [biometricModalOpen, setBiometricModalOpen] = useState(false);
  const [biometricForm, setBiometricForm] = useState({ document_title: '', document_type: 'defense_memo', approver_name: '', approver_role: '', biometric_method: 'fingerprint' });

  const fetchAll = useCallback(async () => {
    setLoading(true);
    const [caseRes, attRes, dlRes, revRes, bioRes, auditRes] = await Promise.all([
      supabase.from('m02_admin_cases')
        .select('*, advisor:lf_attorneys(name)')
        .order('created_at', { ascending: false }),
      supabase.from('lf_attorneys').select('*').order('name'),
      supabase.from('m02_admin_deadlines').select('*').order('deadline_date', { ascending: true }),
      supabase.from('m02_constitutional_reviews').select('*').order('created_at', { ascending: false }),
      supabase.from('m02_biometric_approvals').select('*').order('created_at', { ascending: false }),
      supabase.from('m02_admin_audit_logs').select('*').order('created_at', { ascending: false }).limit(50),
    ]);
    setCases((caseRes.data as M02AdminCase[]) || []);
    setAttorneys((attRes.data as Attorney[]) || []);
    setAllDeadlines((dlRes.data as M02AdminDeadline[]) || []);
    setAllReviews((revRes.data as M02ConstitutionalReview[]) || []);
    setAllBiometrics((bioRes.data as M02BiometricApproval[]) || []);
    setAllAudit((auditRes.data as M02AdminAuditLog[]) || []);
    setLoading(false);
  }, []);
  ...
  const logAudit = async (caseId: string, action: string, detail: string) => {
    ...
    await supabase.from('m02_admin_audit_logs').insert({
  ...
  const openEdit = (c: M02AdminCase) => {
    setForm({
      ...
      court_fees: String(c.court_fees || 0), assigned_advisor_id: c.assigned_advisor_id || '',
  ...
  const handleSave = async () => {
    ...
    if (editingId) {
      await supabase.from('m02_admin_cases').update(payload).eq('id', editingId);
      await logAudit(editingId, 'case_updated', 'تحديث بيانات القضية الإدارية');
    } else {
      const { data } = await supabase.from('m02_admin_cases').insert(payload).select('id');
      const newId = data?.[0]?.id;
      if (newId) {
        await logAudit(newId, 'case_created', 'إنشاء ملف قضية إدارية — تصنيف: ' + (CATEGORY_LABELS[form.case_category] || form.case_category));
        await supabase.from('m02_admin_cases').update({
          m10_linked: true,
          m54_cost_center_opened: true,
          m102_notified: true,
          cost_center_id: 'CC-M02-' + Date.now().toString().slice(-6),
        }).eq('id', newId);
        ...
        await logAudit(newId, 'm102_notified', 'إخطار الإدارات القانونية والمالية عبر محرك التكامل (M102)');
        if (form.challenged_decision_date) {
          ...
          await supabase.from('m02_admin_deadlines').insert({
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    if (deleteType === 'case') await supabase.from('m02_admin_cases').delete().eq('id', deleteId);
    else if (deleteType === 'party') await supabase.from('m02_admin_parties').delete().eq('id', deleteId);
    else if (deleteType === 'deadline') await supabase.from('m02_admin_deadlines').delete().eq('id', deleteId);
    else if (deleteType === 'review') await supabase.from('m02_constitutional_reviews').delete().eq('id', deleteId);
    else if (deleteType === 'biometric') await supabase.from('m02_biometric_approvals').delete().eq('id', deleteId);
    setDeleteId(null);
  ...
  const openCaseDetail = async (c: M02AdminCase) => {
    setSelectedCase(c);
    setDetailLoading(true);
    const [pRes, dlRes, rRes, bRes, aRes] = await Promise.all([
      supabase.from('m02_admin_parties').select('*').eq('case_id', c.id).order('created_at', { ascending: true }),
      supabase.from('m02_admin_deadlines').select('*').eq('case_id', c.id).order('deadline_date', { ascending: true }),
      supabase.from('m02_constitutional_reviews').select('*').eq('case_id', c.id).order('created_at', { ascending: false }),
      supabase.from('m02_biometric_approvals').select('*').eq('case_id', c.id).order('created_at', { ascending: false }),
      supabase.from('m02_admin_audit_logs').select('*').eq('case_id', c.id).order('created_at', { ascending: true }),
    ]);
    setParties((pRes.data as M02AdminParty[]) || []);
    setDeadlines((dlRes.data as M02AdminDeadline[]) || []);
    setReviews((rRes.data as M02ConstitutionalReview[]) || []);
    setBiometrics((bRes.data as M02BiometricApproval[]) || []);
    setAuditLogs((aRes.data as M02AdminAuditLog[]) || []);
    setDetailLoading(false);
  };

  const advanceStage = async (c: M02AdminCase) => {
    const idx = STAGES.indexOf(c.stage);
    if (idx < 0 || idx >= STAGES.length - 1) return;
    const next = STAGES[idx + 1];
    await supabase.from('m02_admin_cases').update({ stage: next }).eq('id', c.id);
    await logAudit(c.id, 'stage_advanced', 'تقدم المرحلة: ' + (STAGE_CONFIG[next]?.label || next));
    fetchAll();
    const updated = { ...c, stage: next };
    setSelectedCase(updated as M02AdminCase);
  };

  const addParty = async () => {
    if (!selectedCase || !partyForm.name.trim()) return;
    await supabase.from('m02_admin_parties').insert({
  ...
  const addReview = async () => {
    if (!selectedCase || !reviewForm.subject_title.trim()) return;
    await supabase.from('m02_constitutional_reviews').insert({
      case_id: selectedCase.id, review_type: reviewForm.review_type,
      subject_title: reviewForm.subject_title.trim(),
      subject_ref: reviewForm.subject_ref.trim() || null,
      constitutional_principle: reviewForm.constitutional_principle.trim() || null,
      findings: reviewForm.findings.trim() || null,
      precedent_refs: reviewForm.precedent_refs.split('\n').filter(Boolean),
      review_result: 'pending', compliance_status: 'pending',
    });
    await logAudit(selectedCase.id, 'constitutional_review', 'فحص دستوري: ' + reviewForm.subject_title);
    setReviewForm({ review_type: 'decision_review', subject_title: '', subject_ref: '', constitutional_principle: '', findings: '', precedent_refs: '' });
    setReviewModalOpen(false);
    openCaseDetail(selectedCase);
  };

  const addBiometric = async () => {
    if (!selectedCase || !biometricForm.document_title.trim() || !biometricForm.approver_name.trim()) return;
    await supabase.from('m02_biometric_approvals').insert({
      case_id: selectedCase.id, document_title: biometricForm.document_title.trim(),
      document_type: biometricForm.document_type, approver_name: biometricForm.approver_name.trim(),
      approver_role: biometricForm.approver_role.trim() || null,
      biometric_method: biometricForm.biometric_method, approval_status: 'pending',
    });
    await logAudit(selectedCase.id, 'biometric_requested', 'طلب مصادقة بيومترية على: ' + biometricForm.document_title);
    setBiometricForm({ document_title: '', document_type: 'defense_memo', approver_name: '', approver_role: '', biometric_method: 'fingerprint' });
    setBiometricModalOpen(false);
    openCaseDetail(selectedCase);
  };

  const approveBiometric = async (b: M02BiometricApproval) => {
    const hash = '0x' + Math.random().toString(16).substr(2, 4) + '...' + Math.random().toString(16).substr(2, 4);
    await supabase.from('m02_biometric_approvals').update({
      approval_status: 'approved', approved_at: new Date().toISOString(), biometric_hash: hash,
    }).eq('id', b.id);
    if (selectedCase) await logAudit(selectedCase.id, 'biometric_approval', 'مصادقة بيومترية على: ' + b.document_title + ' — ' + (BIOMETRIC_METHOD_LABELS[b.biometric_method] || b.biometric_method));
    if (selectedCase) openCaseDetail(selectedCase);
  };

  const completeDeadline = async (d: M02AdminDeadline) => {
    await supabase.from('m02_admin_deadlines').update({
  ...
  const pendingBio = allBiometrics.filter((b) => b.approval_status === 'pending').length;
  ...
  const tabs: { id: Tab; label: string; icon: typeof Landmark; badge?: number }[] = [
    { id: 'cases', label: 'الدعاوى الإدارية', icon: Landmark, badge: activeCases },
    { id: 'deadlines', label: 'المواعيد الإجرائية', icon: Calendar, badge: upcomingDl },
    { id: 'constitutional', label: 'الرقابة الدستورية', icon: Scale },
    { id: 'biometric', label: 'المصادقة البيومترية', icon: Fingerprint, badge: pendingBio },
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-heading font-bold text-midnight text-lg">محرك القضاء الإداري والدستوري (M2)</h2>
            <p className="font-body text-[10px] text-ink/40">القطاع القضائي والإجرائي — المنازعات التي تكون الدولة طرفاً فيها</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        <StatCard icon={<Fingerprint size={14} className="text-amber-600" />} label="مصادقات معلقة" value={String(pendingBio)} valueClass="text-amber-700" />
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          {[
            ...
            { icon: CircuitBoard, label: 'محرك التكامل (M102)', desc: 'إخطار الإدارات لحظياً', color: 'text-blue-600' },
            { icon: FileText, label: 'محرك العقود الإدارية (M59)', desc: 'مراجعة امتثال العقود', color: 'text-green-600' },
            { icon: DollarSign, label: 'المحرك المالي (M54)', desc: 'رسوم مجلس الدولة + Hale & Dorr', color: 'text-gold' },
          ].map((item, i) => {
        ...
        <div className="space-y-2">
          {filteredCases.length === 0 ? (
            <div className="text-center py-12 bg-gray-50 rounded-xl border border-gray-200">
              ...
              <p className="font-body text-xs text-ink/30">لا توجد دعاوى إدارية</p>
            </div>
          ) : (
            filteredCases.map((c) => {
                    ...
                    <div className="flex items-start gap-3 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-2 flex-wrap">
                          ...
                          {c.m102_notified && <span className="flex items-center gap-0.5 px-1 py-0.5 rounded text-[9px] font-body bg-amber-50 text-amber-600"><CircuitBoard size={8} /> M102</span>}
                        </div>
                      </div>
                    </div>
      ...
      {/* Constitutional reviews tab */}
      {activeTab === 'constitutional' && (
        <div className="space-y-2">
          {allReviews.length === 0 ? (
            <div className="text-center py-12 bg-gray-50 rounded-xl border border-gray-200"><Scale size={28} className="text-ink/15 mx-auto mb-2" /><p className="font-body text-xs text-ink/30">لا توجد فحوصات دستورية</p></div>
          ) : (
            allReviews.map((r) => {
              const cfg = REVIEW_RESULT_CONFIG[r.review_result] || REVIEW_RESULT_CONFIG.pending;
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-3 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="font-body text-xs font-bold text-midnight mt-1">{r.subject_title}</p>
                        {r.constitutional_principle && <p className="font-body text-[10px] text-purple-600 mt-1 leading-relaxed">{r.constitutional_principle}</p>}
                        {r.findings && <p className="font-body text-[10px] text-ink/50 mt-1 leading-relaxed line-clamp-2">{r.findings}</p>}
                        {r.precedent_refs && r.precedent_refs.length > 0 && (
                          <div className="flex items-center gap-1 mt-2 flex-wrap">
                            <BookOpen size={10} className="text-ink/30" />
                            {r.precedent_refs.map((p, i) => (
                              <span key={i} className="font-body text-[9px] text-ink/40 bg-gray-50 px-1.5 py-0.5 rounded">{p}</span>
                            ))}
                          </div>
                        )}
                      </div>
                    </div>
                    <button onClick={() => { setDeleteId(r.id); setDeleteType('review'); }} className="p-1.5 rounded text-ink/40 hover:text-red-500 hover:bg-red-50 transition-colors opacity-0 group-hover:opacity-100">
                      <Trash2 size={12} />
                    </button>
                  </div>
                </div>
      ...
      {activeTab === 'biometric' && (
        <div className="space-y-2">
          {allBiometrics.length === 0 ? (
            <div className="text-center py-12 bg-gray-50 rounded-xl border border-gray-200"><Fingerprint size={28} className="text-ink/15 mx-auto mb-2" /><p className="font-body text-xs text-ink/30">لا توجد طلبات مصادقة بيومترية</p></div>
          ) : (
            allBiometrics.map((b) => {
              const cfg = BIOMETRIC_STATUS_CONFIG[b.approval_status] || BIOMETRIC_STATUS_CONFIG.pending;
              const c = cases.find((c) => c.id === b.case_id);
              return (
                <div key={b.id} className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 hover:border-gold/30 transition-colors group">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-3 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-body text-xs font-bold text-midnight">{b.document_title}</p>
                        <div className="flex items-center gap-2 mt-2 flex-wrap">
                          <span className={`px-1.5 py-0.5 rounded text-[9px] font-body font-bold ${cfg.bg} ${cfg.text}`}>{cfg.label}</span>
                          <span className="flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[9px] font-body bg-gray-100 text-ink/50"><ScanLine size={8} /> {BIOMETRIC_METHOD_LABELS[b.biometric_method] || b.biometric_method}</span>
                          {c && <span className="font-body text-[9px] text-gold">{c.case_number}</span>}
                          {b.approver_name && <span className="font-body text-[9px] text-ink/40">{b.approver_name}</span>}
                          {b.approver_role && <span className="font-body text-[9px] text-ink/30">{b.approver_role}</span>}
                          {b.biometric_hash && <span className="flex items-center gap-0.5 font-body text-[9px] text-green-600"><Lock size={8} /> {b.biometric_hash}</span>}
                        </div>
                      </div>
                    </div>
                    {b.approval_status === 'pending' && (
                      <button onClick={() => approveBiometric(b)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-green-600 text-white font-body text-[10px] font-bold hover:bg-green-700 transition-colors">
                        <Fingerprint size={12} /> مصادقة
                      </button>
                    )}
                  </div>
                </div>
                ...
                <div key={log.id} className="flex items-start gap-3 px-4 py-3 hover:bg-gray-50 transition-colors">
                  <div className="w-7 h-7 rounded-lg bg-gray-100 flex items-center justify-center flex-shrink-0">
                    {log.action.includes('created') ? <FileText size={12} className="text-blue-600" />
                      ...
                      : log.action.includes('m59') ? <FileText size={12} className="text-blue-600" />
                      : log.action.includes('m102') ? <CircuitBoard size={12} className="text-amber-600" />
                      : log.action.includes('deadline') ? <Calendar size={12} className="text-amber-600" />
                      : log.action.includes('biometric') ? <Fingerprint size={12} className="text-green-600" />
                      : log.action.includes('constitutional') ? <Scale size={12} className="text-purple-600" />
          ...
          <div className="relative bg-white w-full max-w-lg h-full overflow-y-auto shadow-2xl flex flex-col">
            <div className="sticky top-0 bg-white border-b border-gray-100 px-5 py-4 flex items-center justify-between z-10">
              <div className="flex items-center gap-2">
                ...
                <span className="font-heading font-bold text-midnight text-sm">ملف الدعوى الإدارية</span>
              </div>
            ...
            {detailLoading ? (
              ...
              <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-body font-bold ${selectedCase.m102_notified ? 'bg-amber-50 text-amber-600' : 'bg-gray-100 text-ink/30'}`}><CircuitBoard size={10} /> M102 {selectedCase.m102_notified ? 'مُخطَر' : 'غير مُخطَر'}</span>
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-1.5"><Scale size={12} className="text-gold" /><span className="font-body text-[10px] font-bold text-midnight">الفحوصات الدستورية</span></div>
                    <button onClick={() => setReviewModalOpen(true)} className="flex items-center gap-1 px-2 py-1 rounded bg-gold/10 text-gold font-body text-[10px] font-bold hover:bg-gold/20 transition-colors"><Plus size={10} /> فحص جديد</button>
                  </div>
                  <div className="space-y-1.5">
                    {reviews.map((r) => {
                      const cfg = REVIEW_RESULT_CONFIG[r.review_result] || REVIEW_RESULT_CONFIG.pending;
                      return (
                        <div key={r.id} className="px-3 py-2 rounded-lg bg-gray-50 border border-gray-100 group/rev">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="px-1.5 py-0.5 rounded text-[9px] font-body bg-gray-200 text-ink/50">{REVIEW_TYPE_LABELS[r.review_type] || r.review_type}</span>
                            <span className={`px-1.5 py-0.5 rounded text-[9px] font-body font-bold ${cfg.bg} ${cfg.text}`}>{cfg.label}</span>
                          </div>
                          <p className="font-body text-[10px] font-bold text-midnight">{r.subject_title}</p>
                          {r.constitutional_principle && <p className="font-body text-[9px] text-purple-600 mt-0.5">{r.constitutional_principle}</p>}
                          {r.findings && <p className="font-body text-[9px] text-ink/50 mt-0.5 leading-tight">{r.findings}</p>}
                        </div>
                      );
                    })}
                    {reviews.length === 0 && <p className="font-body text-[10px] text-ink/30">لا توجد فحوصات دستورية</p>}
                  </div>
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-1.5"><Fingerprint size={12} className="text-gold" /><span className="font-body text-[10px] font-bold text-midnight">المصادقات البيومترية (M109)</span></div>
                    <button onClick={() => setBiometricModalOpen(true)} className="flex items-center gap-1 px-2 py-1 rounded bg-gold/10 text-gold font-body text-[10px] font-bold hover:bg-gold/20 transition-colors"><Plus size={10} /> طلب مصادقة</button>
                  </div>
                  <div className="space-y-1.5">
                    {biometrics.map((b) => {
                      const cfg = BIOMETRIC_STATUS_CONFIG[b.approval_status] || BIOMETRIC_STATUS_CONFIG.pending;
                      return (
                        <div key={b.id} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-50 border border-gray-100 group/bio">
                          <Fingerprint size={12} className={cfg.text} />
                          <div className="flex-1 min-w-0">
                            <p className="font-body text-[10px] font-bold text-midnight">{b.document_title}</p>
                            <div className="flex items-center gap-1.5">
                              <span className={`px-1 py-0.5 rounded text-[9px] font-body ${cfg.bg} ${cfg.text}`}>{cfg.label}</span>
                              <span className="font-body text-[9px] text-ink/40">{b.approver_name}</span>
                              {b.biometric_hash && <span className="font-body text-[9px] text-green-600">{b.biometric_hash}</span>}
                            </div>
                          </div>
                          {b.approval_status === 'pending' && <button onClick={() => approveBiometric(b)} className="flex items-center gap-1 px-2 py-1 rounded bg-green-600 text-white font-body text-[9px] font-bold hover:bg-green-700 transition-colors"><Fingerprint size={9} /> مصادقة</button>}
                        </div>
                      );
                    })}
                    {biometrics.length === 0 && <p className="font-body text-[10px] text-ink/30">لا توجد مصادقات بيومترية</p>}
                  </div>
                </div>
      ...
      <EntityModal open={modalOpen} title={editingId ? 'تعديل الدعوى الإدارية' : 'دعوى إدارية جديدة'} onClose={() => setModalOpen(false)} onSubmit={handleSave} loading={saving}>
        <div className="grid grid-cols-2 gap-4">
          <Field label="رقم الدعوى" required><TextInput value={form.case_number} onChange={(e) => setForm({ ...form, case_number: e.target.value })} placeholder="ADMIN-2025-001" /></Field>
          <Field label="الفئة">
      ...
      {/* Constitutional review modal */}
      <EntityModal open={reviewModalOpen} title="فحص دستوري جديد" onClose={() => setReviewModalOpen(false)} onSubmit={addReview}>
        ...
        <Field label="عنوان الموضوع" required><TextInput value={reviewForm.subject_title} onChange={(e) => setReviewForm({ ...reviewForm, subject_title: e.target.value })} /></Field>
        <Field label="المرجع"><TextInput value={reviewForm.subject_ref} onChange={(e) => setReviewForm({ ...reviewForm, subject_ref: e.target.value })} /></Field>
        <Field label="المبدأ الدستوري"><TextInput value={reviewForm.constitutional_principle} onChange={(e) => setReviewForm({ ...reviewForm, constitutional_principle: e.target.value })} /></Field>
        ...
        <Field label="السوابق القضائية (سطر لكل سابقة)"><TextArea value={reviewForm.precedent_refs} onChange={(e) => setReviewForm({ ...reviewForm, precedent_refs: e.target.value })} rows={3} /></Field>
      </EntityModal>

      {/* Biometric modal */}
      <EntityModal open={biometricModalOpen} title="طلب مصادقة بيومترية" onClose={() => setBiometricModalOpen(false)} onSubmit={addBiometric}>
        <Field label="عنوان المستند" required><TextInput value={biometricForm.document_title} onChange={(e) => setBiometricForm({ ...biometricForm, document_title: e.target.value })} /></Field>
        <div className="grid grid-cols-2 gap-4">
          <Field label="نوع المستند">
            <Select value={biometricForm.document_type} onChange={(e) => setBiometricForm({ ...biometricForm, document_type: e.target.value })}>
              <option value="defense_memo">مذكرة دفاع</option>
              <option value="appeal_memo">مذكرة طعن</option>
              <option value="appeal_brief">صحيفة طعن</option>
              <option value="compensation_memo">مذكرة مطالبة</option>
              <option value="expert_report">تقرير خبير</option>
            </Select>
          </Field>
          <Field label="طريقة المصادقة">
            <Select value={biometricForm.biometric_method} onChange={(e) => setBiometricForm({ ...biometricForm, biometric_method: e.target.value })}>
              {Object.entries(BIOMETRIC_METHOD_LABELS).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
            </Select>
          </Field>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Field label="اسم المستشار" required><TextInput value={biometricForm.approver_name} onChange={(e) => setBiometricForm({ ...biometricForm, approver_name: e.target.value })} /></Field>
          <Field label="صفة المستشار"><TextInput value={biometricForm.approver_role} onChange={(e) => setBiometricForm({ ...biometricForm, approver_role: e.target.value })} /></Field>
        </div>
      </EntityModal>
