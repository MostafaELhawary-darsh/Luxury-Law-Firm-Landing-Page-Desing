...
import {
  ...
  FileText, Scale, HardHat, FlaskConical, Footprints,
  Users, Radio,
} from 'lucide-react';
import { supabase, formatDate, formatCurrency } from '@/lib/financeUtils';
import type {
  M91HseFile, M91AuditLog,
} from '@/lib/firmTypes';
...
const emptyForm: HseFileForm = {
  file_number: '', file_title: '', file_type: 'safety_license', stage: 'draft',
  facility_name: '', site_type: 'factory', license_number: '', license_type: '',
  risk_level: 'low', incident_reported: false, incident_type: '', injuries_count: '0',
  fatalities: '0', hazardous_materials: false, hazmat_permit_ref: '',
  osha_compliant: true, evacuation_plan: false, compensation_amount: '0',
  currency: 'SAR', insurance_claim: '0',
  description: '',
};

export default function HealthSafetyEngine({ voiceAdd }: { voiceAdd?: () => PendingAddCommand | null }) {
  const [files, setFiles] = useState<M91HseFile[]>([]);
  const [attorneys, setAttorneys] = useState<Attorney[]>([]);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<Tab>('files');
  const [selectedFile, setSelectedFile] = useState<M91HseFile | null>(null);
  const [auditLogs, setAuditLogs] = useState<M91AuditLog[]>([]);
  const [allAudit, setAllAudit] = useState<M91AuditLog[]>([]);
  const [detailLoading, setDetailLoading] = useState(false);
  const [modalOpen, setModalOpen] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [form, setForm] = useState<HseFileForm>(emptyForm);
  ...
  const fetchAll = useCallback(async () => {
    ...
    const [fRes, attRes, auditRes] = await Promise.all([
      supabase.from('m91_hse_files')
        .select('*, advisor:lf_attorneys(name)')
        .order('created_at', { ascending: false }),
      supabase.from('lf_attorneys').select('*').order('name'),
      supabase.from('m91_audit_logs').select('*').order('created_at', { ascending: false }).limit(50),
    ]);
    if (fRes.error) console.error('m91 fetch error', fRes.error);
    ...
    setFiles((fRes.data as M91HseFile[]) || []);
    setAttorneys((attRes.data as Attorney[]) || []);
    setAllAudit((auditRes.data as M91AuditLog[]) || []);
    setLoading(false);
  }, []);
  ...
  const logAudit = async (fileId: string, action: string, detail: string) => {
    ...
    const { error } = await supabase.from('m91_audit_logs').insert({
  ...
  const openEdit = (f: M91HseFile) => {
    setForm({
      file_number: f.file_number, file_title: f.file_title,
      file_type: f.file_type, stage: f.stage,
      facility_name: f.facility_name || '', site_type: f.site_type || 'factory',
      license_number: f.license_number || '', license_type: f.license_type || '',
      risk_level: f.risk_level || 'low', incident_reported: !!f.incident_reported,
      incident_type: f.incident_type || '', injuries_count: String(f.injuries_count || 0),
      fatalities: String(f.fatalities || 0), hazardous_materials: !!f.hazardous_materials,
      hazmat_permit_ref: f.hazmat_permit_ref || '',
      osha_compliant: !!f.osha_compliant, evacuation_plan: !!f.evacuation_plan,
      compensation_amount: String(f.compensation_amount || 0),
      currency: f.currency || 'SAR', insurance_claim: String(f.insurance_claim || 0),
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    const injuries = Number(form.injuries_count) || 0;
    const fatalities = Number(form.fatalities) || 0;
    const compensation = Number(form.compensation_amount) || 0;
    const insurance = Number(form.insurance_claim) || 0;
    const payload = {
      ...
      osha_compliant: form.osha_compliant,
      evacuation_plan: form.evacuation_plan,
    ...
    if (editingId) {
      const { error } = await supabase.from('m91_hse_files').update(payload).eq('id', editingId);
      if (error) console.error('update error', error);
      await logAudit(editingId, 'file_updated', 'تحديث بيانات ملف الصحة والسلامة المهنية');
    } else {
      const { data, error } = await supabase.from('m91_hse_files').insert(payload).select('id');
      if (error) console.error('insert error', error);
      const newId = data?.[0]?.id;
      if (newId) {
        await logAudit(newId, 'file_created', 'إنشاء ملف سلامة — النوع: ' + (FILE_TYPE_LABELS[form.file_type] || form.file_type));
        const needsCase = form.file_type === 'incident' || fatalities > 0;
        await supabase.from('m91_hse_files').update({
          m53_document_id: 'DOC-M91-' + Date.now().toString().slice(-6),
          ...
          m92_notified: true,
          cost_center_id: 'CC-M91-' + Date.now().toString().slice(-6),
        }).eq('id', newId);
        await logAudit(newId, 'm53_document', 'أرشفة التقارير في محرك المستندات (M53)');
        await logAudit(newId, 'm54_finance', 'ربط التعويضات والتأمين بالمحرك المالي (M54)');
        await logAudit(newId, 'm77_hr', 'ربط ملفات الموظفين بمحرك الموارد البشرية (M77)');
        await logAudit(newId, 'm107_iot', 'ربط حساسات الإنذار بإنترنت الأشياء (M107)');
        if (needsCase) await logAudit(newId, 'm10_case', 'فتح قضية حادث العمل في المحرك الموحد (M10)');
        await logAudit(newId, 'm109_biometric', 'توقيع التقارير بيومتري (M109)');
        await logAudit(newId, 'm92_notified', 'إخطار الوكيل الذكي (M92) بالتنبيهات الحرجة');
      }
    }
    setSaving(false);
    setModalOpen(false);
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    const { error } = await supabase.from('m91_hse_files').delete().eq('id', deleteId);
  ...
  const openFileDetail = async (f: M91HseFile) => {
    setSelectedFile(f);
    setDetailLoading(true);
    const aRes = await supabase.from('m91_audit_logs').select('*').eq('case_id', f.id).order('created_at', { ascending: true });
    if (aRes.error) console.error('detail audit error', aRes.error);
    setAuditLogs((aRes.data as M91AuditLog[]) || []);
    setDetailLoading(false);
  };

  const advanceStage = async (f: M91HseFile) => {
    const idx = STAGES.indexOf(f.stage);
    if (idx < 0 || idx >= STAGES.length - 1) return;
    const next = STAGES[idx + 1];
    const { error } = await supabase.from('m91_hse_files').update({ stage: next, status: next === 'terminated' ? 'terminated' : 'active' }).eq('id', f.id);
    ...
    fetchAll();
    setSelectedFile({ ...f, stage: next } as M91HseFile);
  };
  ...
  const filteredFiles = files.filter((f) => {
    ...
    if (searchQuery) {
      ...
      if (!f.file_number.toLowerCase().includes(q) && !f.file_title.toLowerCase().includes(q) && !(f.facility_name || '').toLowerCase().includes(q) && !(f.license_number || '').toLowerCase().includes(q) && !(f.incident_type || '').toLowerCase().includes(q)) return false;
    }
    return true;
  });
  ...
  const incidentReportsCount = files.filter((f) => f.incident_reported).length;
  const totalInjuries = files.reduce((s, f) => s + (f.injuries_count || 0), 0);
  const totalInsuranceClaims = files.reduce((s, f) => s + (f.insurance_claim || 0), 0);

  const tabs: { id: Tab; label: string; icon: typeof Building2; badge?: number }[] = [
    { id: 'files', label: 'ملفات الصحة والسلامة المهنية', icon: HardHat, badge: files.length },
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-heading font-bold text-midnight text-lg">الصحة والسلامة المهنية وتأمين بيئة العمل (M91)</h2>
            <p className="font-body text-[10px] text-ink/40">إدارة تراخيص السلامة وتقييم المخاطر وسجلات الحوادث والامتثال لمعايير OSHA</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard icon={<HardHat size={14} className="text-midnight" />} label="إجمالي الملفات" value={String(files.length)} valueClass="text-midnight" />
        <StatCard icon={<AlertCircle size={14} className="text-red-600" />} label="بلاغات الحوادث" value={String(incidentReportsCount)} valueClass="text-red-600" />
        <StatCard icon={<AlertTriangle size={14} className="text-orange-600" />} label="إجمالي الإصابات" value={String(totalInjuries)} valueClass="text-orange-600" />
        <StatCard icon={<DollarSign size={14} className="text-gold" />} label="مطالبات التأمين" value={formatCurrency(totalInsuranceClaims)} valueClass="text-gold" />
      </div>
      ...
      <div className="bg-midnight rounded-xl p-4 border border-gold/20">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="font-heading font-bold text-cream text-xs">دورة حياة ملف السلامة — 6 مراحل</span>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-7 gap-2">
          {[
            { icon: FileText, label: 'المستندات (M53)', desc: 'أرشفة التقارير', color: 'text-purple-600' },
            { icon: DollarSign, label: 'المحرك المالي (M54)', desc: 'التعويضات والتأمين', color: 'text-gold' },
            { icon: Users, label: 'الموارد البشرية (M77)', desc: 'ملفات الموظفين', color: 'text-green-600' },
            { icon: Radio, label: 'إنترنت الأشياء (M107)', desc: 'حساسات الإنذار', color: 'text-amber-600' },
            { icon: Scale, label: 'نواة القضية (M10)', desc: 'قضايا حوادث العمل', color: 'text-blue-600' },
            { icon: BadgeCheck, label: 'البيومتري (M109)', desc: 'توقيع التقارير', color: 'text-green-600' },
            { icon: Activity, label: 'الوكيل الذكي (M92)', desc: 'تنبيهات حرجة', color: 'text-amber-600' },
          ].map((item, i) => {
        ...
        <div className="space-y-2">
          {filteredFiles.length === 0 ? (
            <div className="text-center py-12 bg-gray-50 rounded-xl border border-gray-200">
              <HardHat size={28} className="text-ink/15 mx-auto mb-2" />
              <p className="font-body text-xs text-ink/30">لا توجد ملفات سلامة مهنية مسجلة</p>
            </div>
          ) : (
            filteredFiles.map((f) => {
              const sCfg = STAGE_CONFIG[f.stage] || STAGE_CONFIG.draft;
              const stageIdx = STAGES.indexOf(f.stage);
              const TypeIcon = FILE_TYPE_ICONS[f.file_type] || HardHat;
              const riskCfg = RISK_LEVEL_CONFIG[f.risk_level || 'low'] || RISK_LEVEL_CONFIG.low;
              return (
                    ...
                    <div className="flex items-start gap-3 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[9px] font-body font-bold ${riskCfg.bg} ${riskCfg.text}`}>
                            <AlertTriangle size={8} className="fill-current" /> {RISK_LEVEL_LABELS[f.risk_level || 'low'] || f.risk_level || '—'}
                          </span>
                          {f.incident_reported && (
                            <span className="flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[9px] font-body font-bold bg-red-50 text-red-600">
                              <AlertCircle size={8} /> حادث
                            </span>
                          )}
                        ...
                        <div className="flex items-center gap-2 mt-2 flex-wrap">
                          ...
                          {f.site_type && <span className="font-body text-[9px] text-ink/40">الموقع: {SITE_TYPE_LABELS[f.site_type] || f.site_type}</span>}
                          {f.license_number && <span className="font-body text-[9px] text-ink/40">ترخيص: {f.license_number}</span>}
                          {f.injuries_count > 0 && <span className="font-body text-[9px] text-orange-600 font-bold">إصابات: {f.injuries_count}</span>}
                          {f.fatalities > 0 && <span className="font-body text-[9px] text-red-600 font-bold">وفيات: {f.fatalities}</span>}
                          {f.compensation_amount > 0 && <span className="font-body text-[9px] text-orange-600 font-bold">تعويض: {formatCurrency(f.compensation_amount)}</span>}
                          {f.insurance_claim > 0 && <span className="font-body text-[9px] text-gold font-bold">تأمين: {formatCurrency(f.insurance_claim)}</span>}
              ...
              {allAudit.map((log) => (
                <div key={log.id} className="flex items-start gap-3 px-4 py-3 hover:bg-gray-50 transition-colors">
                  <div className="w-7 h-7 rounded-lg bg-gray-100 flex items-center justify-center flex-shrink-0">
                    {log.action.includes('created') ? <HardHat size={12} className="text-blue-600" />
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-end">
          ...
          <div className="relative bg-white w-full max-w-lg h-full overflow-y-auto shadow-2xl flex flex-col">
            <div className="sticky top-0 bg-white border-b border-gray-100 px-5 py-4 flex items-center justify-between z-10">
              <div className="flex items-center gap-2">
                <HardHat size={16} className="text-gold" />
                <span className="font-heading font-bold text-midnight text-sm">ملف الصحة والسلامة المهنية</span>
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
                ...
                <div className="bg-gold/5 rounded-lg p-3 border border-gold/10">
                  <div className="flex items-center gap-1.5 mb-2">
                    <HardHat size={12} className="text-gold" />
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div><span className="font-body text-[9px] text-ink/40">نوع الموقع</span><p className="font-body text-xs font-bold text-midnight">{SITE_TYPE_LABELS[selectedFile.site_type || ''] || selectedFile.site_type || '—'}</p></div>
                ...
                {/* Risk level card */}
                <div className={`rounded-lg p-3 border border-current ${(RISK_LEVEL_CONFIG[selectedFile.risk_level || 'low'] || RISK_LEVEL_CONFIG.low).bg}`}>
                  <div className="flex items-center gap-1.5 mb-1">
                    <AlertTriangle size={12} className={(RISK_LEVEL_CONFIG[selectedFile.risk_level || 'low'] || RISK_LEVEL_CONFIG.low).text} />
                    <span className="font-body text-[10px] font-bold text-midnight">مستوى الخطورة</span>
                  </div>
                  <p className={`font-body text-sm font-bold ${(RISK_LEVEL_CONFIG[selectedFile.risk_level || 'low'] || RISK_LEVEL_CONFIG.low).text}`}>
                    {RISK_LEVEL_LABELS[selectedFile.risk_level || 'low'] || selectedFile.risk_level}
                  </p>
                </div>

                {/* Incident card */}
                <div className={`rounded-lg p-3 border ${selectedFile.incident_reported ? 'bg-red-50 border-red-100' : 'bg-gray-100 border-gray-200'}`}>
                  <div className="flex items-center gap-1.5 mb-2">
                    <AlertCircle size={12} className={selectedFile.incident_reported ? 'text-red-600' : 'text-ink/40'} />
                    <span className="font-body text-[10px] font-bold text-midnight">بطاقة الحادث</span>
                  </div>
                  <div className="grid grid-cols-3 gap-2">
                    <div><span className="font-body text-[9px] text-ink/40">نوع الحادث</span><p className="font-body text-xs font-bold text-midnight">{selectedFile.incident_type || '—'}</p></div>
                    <div><span className="font-body text-[9px] text-ink/40">عدد الإصابات</span><p className="font-body text-xs font-bold text-orange-600">{selectedFile.injuries_count || 0}</p></div>
                    <div><span className="font-body text-[9px] text-ink/40">عدد الوفيات</span><p className="font-body text-xs font-bold text-red-600">{selectedFile.fatalities || 0}</p></div>
                  </div>
                </div>

                {/* OSHA compliance card */}
                <div className={`rounded-lg p-3 border ${selectedFile.osha_compliant ? 'bg-green-50 border-green-100' : 'bg-red-50 border-red-100'}`}>
                  <div className="flex items-center gap-1.5 mb-1">
                    {selectedFile.osha_compliant ? <CheckCircle2 size={12} className="text-green-600" /> : <AlertCircle size={12} className="text-red-600" />}
                    <span className="font-body text-[10px] font-bold text-midnight">الامتثال لمعايير OSHA</span>
                  </div>
                  <p className={`font-body text-sm font-bold ${selectedFile.osha_compliant ? 'text-green-700' : 'text-red-700'}`}>
                    {selectedFile.osha_compliant ? 'متوافق' : 'غير متوافق'}
                  </p>
                </div>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  {selectedFile.hazardous_materials && (
                    <span className="flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-body font-bold bg-purple-50 text-purple-600">
                      <FlaskConical size={10} /> مواد خطرة
                    </span>
                  )}
                  {selectedFile.evacuation_plan && (
                    <span className="flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-body font-bold bg-blue-50 text-blue-600">
                      <Footprints size={10} /> خطة إخلاء معتمدة
                    </span>
                  )}
                </div>

                {/* Compensation & insurance card */}
                <div className="bg-orange-50 rounded-lg p-3 border border-orange-100">
                  <div className="flex items-center gap-1.5 mb-2">
                    <DollarSign size={12} className="text-orange-600" />
                    <span className="font-body text-[10px] font-bold text-midnight">التعويضات والتأمين</span>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div><span className="font-body text-[9px] text-ink/40">مبلغ التعويض</span><p className="font-body text-sm font-bold text-orange-700">{formatCurrency(selectedFile.compensation_amount)}</p></div>
                    <div><span className="font-body text-[9px] text-ink/40">مطالبة التأمين</span><p className="font-body text-sm font-bold text-gold">{formatCurrency(selectedFile.insurance_claim)}</p></div>
                  </div>
                </div>
      ...
      <EntityModal open={modalOpen} title={editingId ? 'تعديل الملف' : 'ملف سلامة جديد'} onClose={() => setModalOpen(false)} onSubmit={handleSave} loading={saving}>
        <div className="grid grid-cols-2 gap-4">
          <Field label="رقم الملف" required><TextInput value={form.file_number} onChange={(e) => setForm({ ...form, file_number: e.target.value })} placeholder="HSE-2025-001" /></Field>
        ...
        <div className="flex items-center gap-6">
          ...
          <Checkbox checked={form.osha_compliant} onChange={(v: boolean) => setForm({ ...form, osha_compliant: v })} label="متوافق مع OSHA" />
          <Checkbox checked={form.evacuation_plan} onChange={(v: boolean) => setForm({ ...form, evacuation_plan: v })} label="خطة إخلاء معتمدة" />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Field label="نوع الحادث"><TextInput value={form.incident_type} onChange={(e) => setForm({ ...form, incident_type: e.target.value })} /></Field>
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="عدد الإصابات"><TextInput type="number" value={form.injuries_count} onChange={(e) => setForm({ ...form, injuries_count: e.target.value })} /></Field>
          <Field label="عدد الوفيات"><TextInput type="number" value={form.fatalities} onChange={(e) => setForm({ ...form, fatalities: e.target.value })} /></Field>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Field label="مبلغ التعويض"><TextInput type="number" value={form.compensation_amount} onChange={(e) => setForm({ ...form, compensation_amount: e.target.value })} /></Field>
          <Field label="مطالبة التأمين"><TextInput type="number" value={form.insurance_claim} onChange={(e) => setForm({ ...form, insurance_claim: e.target.value })} /></Field>
        </div>
        <Field label="العملة">
