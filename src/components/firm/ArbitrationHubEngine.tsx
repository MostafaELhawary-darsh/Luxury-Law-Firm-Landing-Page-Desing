...
export default function ArbitrationHubEngine({ voiceAdd }: { voiceAdd?: () => PendingAddCommand | null }) {
  ...
  const tabs: { id: Tab; label: string; icon: typeof Gavel; badge?: number }[] = [
    ...
    { id: 'live', label: 'غرفة التحكيم المباشرة', icon: Video },
  ...
  return (
    <div className="space-y-5">
      ...
      {activeTab === 'live' && (
        <LiveSessionLauncher
          sessionLabel="غرفة التحكيم المباشرة"
          contextType="arbitration_hub"
          sessions={files.map((f) => ({
            id: f.id,
            title: f.file_number || `ملف تحكيم ${f.id.slice(0, 6)}`,
            date: null,
            status: f.stage || 'active',
            participantNames: [f.claimant_name, f.respondent_name, f.tribunal_president].filter(Boolean) as string[],
            documents: [],
          }))}
          onCreateSession={(ctxId) => {
            ...
            if (!f) return null;
            return {
              title: `تحكيم: ${f.file_number || ctxId.slice(0, 6)}`,
              subtitle: f.file_title || 'جلسة تحكيم إلكتروني',
              contextType: 'arbitration_hub',
              contextId: ctxId,
              participants: [
                { name: f.claimant_name || 'المُدَّعي', role: 'مُدَّعي' },
                { name: f.respondent_name || 'المُدَّعى عليه', role: 'مُدَّعى عليه' },
                { name: f.tribunal_president || f.advisor?.name || 'رئيس الهيئة', role: 'رئيس هيئة' },
              ],
              documents: [],
            };
          }}
