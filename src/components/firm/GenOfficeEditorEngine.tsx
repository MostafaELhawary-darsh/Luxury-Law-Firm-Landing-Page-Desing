...
export default function GenOfficeEditorEngine({ voiceAdd }: { voiceAdd?: () => PendingAddCommand | null }) {
  ...
  const startSession = async (doc: SovereignDocument) => {
    ...
    const editorUrl = `http://localhost:8080/editor?token=${sessionToken}&doc=${doc.id}&perm=${sessionPermissions}`;

    const { data } = await supabase.from('m114_editor_sessions').insert({
    ...
    if (data) {
      ...
      const newHash = `${lastHash}-${Date.now()}`.split('').reduce((h, c) => { h = ((h << 5) - h) + c.charCodeAt(0); return Math.abs(h); }, 0).toString(16);
      await supabase.from('m114_editor_audit').insert({
        session_id: data.id, document_id: doc.id, action: 'session_opened',
        actor: 'current-user', actor_role: 'editor',
        detail: `جلسة تحرير مفتوحة بصلاحية: ${PERMISSION_LABELS[sessionPermissions] || sessionPermissions}`,
        hash_chain: newHash, previous_hash: lastHash,
        accessed_fields: ['session_token', 'permissions'],
      });
      setActiveSession(data as EditorSession);
      fetchAll();
    }
  };
