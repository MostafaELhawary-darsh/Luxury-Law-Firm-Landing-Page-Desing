...
import {
  ...
  FileText, Scale, Gavel, Users, Landmark, BarChart3, TrendingUp,
} from 'lucide-react';
import { supabase, formatDate, formatCurrency } from '@/lib/financeUtils';
import type {
  M67EconomicFile, M67AuditLog,
} from '@/lib/firmTypes';
...
const STAGE_CONFIG: Record<string, { label: string; bg: string; text: string }> = {
  ...
  audited: { label: 'تدقيق', bg: 'bg-cyan-50', text: 'text-cyan-700' },
  executed: { label: 'منفَّذ', bg: 'bg-green-50', text: 'text-green-700' },
  terminated: { label: 'منتهٍ', bg: 'bg-gray-100', text: 'text-gray-700' },
};

const STAGES = ['draft', 'reviewed', 'audited', 'executed', 'terminated'];

const FILE_TYPE_LABELS: Record<string, string> = {
  ma_agreement: 'اتفاقية استحواذ',
  holding_governance: 'حوكمة قابضة',
  fund_management: 'إدارة صناديق',
  investment_dispute: 'نزاع استثماري',
};

const FILE_TYPE_ICONS: Record<string, typeof Building2> = {
  ma_agreement: TrendingUp,
  holding_governance: Landmark,
  fund_management: BarChart3,
  investment_dispute: Gavel,
};

const FUND_TYPE_LABELS: Record<string, string> = {
  private_equity: 'أسهم خاصة',
  venture_capital: 'رأس مال مخاطر',
  syndicated_loan: 'تمويل مجمع',
  securitization: 'توريق',
};
...
const emptyForm: FileForm = {
  file_number: '', file_title: '', file_type: 'ma_agreement', stage: 'draft',
  entity_name: '', holding_structure: '', deal_value: '0',
  due_diligence_ref: '', fund_type: 'private_equity', arbitration_ref: '', description: '',
};

export default function EconomicInvestmentEngine({ voiceAdd }: { voiceAdd?: () => PendingAddCommand | null }) {
  const [files, setFiles] = useState<M67EconomicFile[]>([]);
  const [attorneys, setAttorneys] = useState<Attorney[]>([]);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<Tab>('files');
  const [selectedFile, setSelectedFile] = useState<M67EconomicFile | null>(null);
  const [auditLogs, setAuditLogs] = useState<M67AuditLog[]>([]);
  const [allAudit, setAllAudit] = useState<M67AuditLog[]>([]);
  ...
  const fetchAll = useCallback(async () => {
    ...
    const [fRes, attRes, auditRes] = await Promise.all([
      supabase.from('m67_economic_files')
        .select('*, advisor:lf_attorneys(name)')
        .order('created_at', { ascending: false }),
      supabase.from('lf_attorneys').select('*').order('name'),
      supabase.from('m67_audit_logs').select('*').order('created_at', { ascending: false }).limit(50),
    ]);
    if (fRes.error) console.error('m67 fetch error', fRes.error);
    ...
    setFiles((fRes.data as M67EconomicFile[]) || []);
    setAttorneys((attRes.data as Attorney[]) || []);
    setAllAudit((auditRes.data as M67AuditLog[]) || []);
    setLoading(false);
  }, []);
  ...
  const logAudit = async (fileId: string, action: string, detail: string) => {
    ...
    const { error } = await supabase.from('m67_audit_logs').insert({
  ...
  const openEdit = (f: M67EconomicFile) => {
    setForm({
      file_number: f.file_number, file_title: f.file_title,
      file_type: f.file_type, stage: f.stage,
      entity_name: f.entity_name ?? '', holding_structure: f.holding_structure || '',
      deal_value: String(f.deal_value || 0),
      due_diligence_ref: f.due_diligence_ref || '', fund_type: f.fund_type || 'private_equity',
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      holding_structure: form.holding_structure.trim() || null,
      deal_value: value,
      due_diligence_ref: form.due_diligence_ref.trim() || null,
      fund_type: form.fund_type,
      arbitration_ref: form.arbitration_ref.trim() || null,
      description: form.description.trim() || null,
    };
    if (editingId) {
      const { error } = await supabase.from('m67_economic_files').update(payload).eq('id', editingId);
      if (error) console.error('update error', error);
      await logAudit(editingId, 'file_updated', 'تحديث بيانات الملف الاقتصادي');
    } else {
      const { data, error } = await supabase.from('m67_economic_files').insert(payload).select('id');
      if (error) console.error('insert error', error);
      const newId = data?.[0]?.id;
      if (newId) {
        await logAudit(newId, 'file_created', 'إنشاء ملف اقتصادي — النوع: ' + (FILE_TYPE_LABELS[form.file_type] || form.file_type));
        await supabase.from('m67_economic_files').update({
          m53_document_id: 'DOC-M67-' + Date.now().toString().slice(-6),
          m54_finance_linked: true,
          m10_case_opened: true,
          m98_market_linked: form.file_type === 'holding_governance' || form.file_type === 'ma_agreement',
          m77_hr_linked: form.file_type === 'ma_agreement',
          m109_biometric_signed: true,
          m92_notified: true,
          cost_center_id: 'CC-M67-' + Date.now().toString().slice(-6),
        }).eq('id', newId);
        ...
        if (form.file_type === 'holding_governance' || form.file_type === 'ma_agreement') await logAudit(newId, 'm98_market', 'ربط الملف بمحرك أسواق المال (M98)');
        if (form.file_type === 'ma_agreement') await logAudit(newId, 'm77_hr', 'ربط الملف بمحرك الموارد البشرية (M77)');
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    const { error } = await supabase.from('m67_economic_files').delete().eq('id', deleteId);
  ...
  const openFileDetail = async (f: M67EconomicFile) => {
    setSelectedFile(f);
    setDetailLoading(true);
    const aRes = await supabase.from('m67_audit_logs').select('*').eq('case_id', f.id).order('created_at', { ascending: true });
    if (aRes.error) console.error('detail audit error', aRes.error);
    setAuditLogs((aRes.data as M67AuditLog[]) || []);
    setDetailLoading(false);
  };

  const advanceStage = async (f: M67EconomicFile) => {
    const idx = STAGES.indexOf(f.stage);
    if (idx < 0 || idx >= STAGES.length - 1) return;
    const next = STAGES[idx + 1];
    const { error } = await supabase.from('m67_economic_files').update({ stage: next, status: next === 'terminated' ? 'terminated' : 'active' }).eq('id', f.id);
    ...
    fetchAll();
    setSelectedFile({ ...f, stage: next } as M67EconomicFile);
  };
  ...
  const filteredFiles = files.filter((f) => {
    ...
    if (searchQuery) {
      ...
      if (!f.file_number.toLowerCase().includes(q) && !f.file_title.toLowerCase().includes(q) && !(f.entity_name ?? '').toLowerCase().includes(q)) return false;
    }
    return true;
  });
  ...
  const activeMAndA = files.filter((f) => f.file_type === 'ma_agreement' && f.stage !== 'terminated').length;
  const totalDealValue = files.reduce((s, f) => s + (f.deal_value || 0), 0);
  const fundCount = files.filter((f) => f.file_type === 'fund_management').length;

  const tabs: { id: Tab; label: string; icon: typeof Building2; badge?: number }[] = [
    { id: 'files', label: 'الملفات', icon: Building2, badge: files.length },
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-heading font-bold text-midnight text-lg">المؤسسات الاقتصادية والتجارية والاستثمارية الكبرى (M67)</h2>
            <p className="font-body text-[10px] text-ink/40">إدارة الكيانات القابضة والصناديق الاستثمارية وعمليات الاستحواذ والتمويل الهيكلي</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <StatCard icon={<TrendingUp size={14} className="text-blue-600" />} label="صفقات استحواذ نشطة" value={String(activeMAndA)} valueClass="text-blue-700" />
        <StatCard icon={<DollarSign size={14} className="text-gold" />} label="إجمالي قيمة الصفقات" value={formatCurrency(totalDealValue)} valueClass="text-gold" />
        <StatCard icon={<BarChart3 size={14} className="text-green-600" />} label="ملفات إدارة الصناديق" value={String(fundCount)} valueClass="text-green-700" />
      </div>
      ...
      <div className="bg-midnight rounded-xl p-4 border border-gold/20">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="font-heading font-bold text-cream text-xs">دورة حياة الملف الاقتصادي — 5 مراحل</span>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-7 gap-2">
          {[
            ...
            { icon: BarChart3, label: 'أسواق المال (M98)', desc: 'حوكمة/استحواذ', color: 'text-cyan-600' },
            { icon: Users, label: 'الموارد البشرية (M77)', desc: 'استحواذ', color: 'text-green-600' },
      ...
      {activeTab === 'files' && (
        <div className="flex items-center gap-2">
          ...
          <div className="relative flex-1 max-w-xs">
            ...
            <TextInput value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="بحث برقم الملف أو العنوان أو الكيان..." className="!py-1.5 !text-xs pr-9" />
          </div>
        </div>
      )}
                    ...
                    <div className="flex items-start gap-3 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {f.fund_type && <span className="px-1.5 py-0.5 rounded text-[9px] font-body bg-cyan-50 text-cyan-600">{FUND_TYPE_LABELS[f.fund_type] || f.fund_type}</span>}
                        </div>
                        <p className="font-body text-xs font-bold text-midnight mt-1 leading-snug line-clamp-2">{f.file_title}</p>
                        <div className="flex items-center gap-2 mt-2 flex-wrap">
                          <span className="font-body text-[9px] text-ink/40">الكيان: {f.entity_name}</span>
                          {f.holding_structure && <span className="font-body text-[9px] text-ink/40">الهيكل: {f.holding_structure}</span>}
                          {f.deal_value > 0 && <span className="font-body text-[9px] text-gold font-bold">{formatCurrency(f.deal_value)}</span>}
                          {f.due_diligence_ref && <span className="font-body text-[9px] text-ink/40">العناية الواجبة: {f.due_diligence_ref}</span>}
                          {f.arbitration_ref && <span className="font-body text-[9px] text-red-600 font-bold">تحكيم: {f.arbitration_ref}</span>}
                          ...
                          {f.m98_market_linked && <span className="flex items-center gap-0.5 px-1 py-0.5 rounded text-[9px] font-body bg-cyan-50 text-cyan-600"><BarChart3 size={8} /> M98</span>}
                ...
                <div key={log.id} className="flex items-start gap-3 px-4 py-3 hover:bg-gray-50 transition-colors">
                  <div className="w-7 h-7 rounded-lg bg-gray-100 flex items-center justify-center flex-shrink-0">
                    {log.action.includes('created') ? <Building2 size={12} className="text-blue-600" />
                      ...
                      : log.action.includes('m98') ? <BarChart3 size={12} className="text-cyan-600" />
          ...
          <div className="relative bg-white w-full max-w-lg h-full overflow-y-auto shadow-2xl flex flex-col">
            <div className="sticky top-0 bg-white border-b border-gray-100 px-5 py-4 flex items-center justify-between z-10">
              <div className="flex items-center gap-2">
                ...
                <span className="font-heading font-bold text-midnight text-sm">ملف المؤسسة الاقتصادية</span>
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
                ...
                <div className="bg-gold/5 rounded-lg p-3 border border-gold/10">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div><span className="font-body text-[9px] text-ink/40">الكيان</span><p className="font-body text-xs font-bold text-midnight">{selectedFile.entity_name}</p></div>
                    <div><span className="font-body text-[9px] text-ink/40">الهيكل القابض</span><p className="font-body text-xs font-bold text-midnight">{selectedFile.holding_structure || '—'}</p></div>
                ...
                <div className="bg-gold/5 rounded-lg p-3 border border-gold/10">
                  ...
                  <span className="font-body text-[9px] text-ink/40">قيمة الصفقة</span>
                  <p className="font-body text-sm font-bold text-gold">{formatCurrency(selectedFile.deal_value)}</p>
                </div>
                ...
                <div className="bg-gray-50 rounded-lg p-3 border border-gray-100">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div><span className="font-body text-[9px] text-ink/40">العناية الواجبة</span><p className="font-body text-xs font-bold text-midnight">{selectedFile.due_diligence_ref || '—'}</p></div>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-body font-bold ${selectedFile.m98_market_linked ? 'bg-cyan-50 text-cyan-600' : 'bg-gray-100 text-ink/30'}`}><BarChart3 size={10} /> M98 {selectedFile.m98_market_linked ? 'مربوط' : 'غير مربوط'}</span>
      ...
      <EntityModal open={modalOpen} title={editingId ? 'تعديل الملف' : 'ملف اقتصادي جديد'} onClose={() => setModalOpen(false)} onSubmit={handleSave} loading={saving}>
        <div className="grid grid-cols-2 gap-4">
          <Field label="رقم الملف" required><TextInput value={form.file_number} onChange={(e) => setForm({ ...form, file_number: e.target.value })} placeholder="M67-2025-001" /></Field>
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="اسم الكيان" required><TextInput value={form.entity_name} onChange={(e) => setForm({ ...form, entity_name: e.target.value })} /></Field>
          <Field label="الهيكل القابض"><TextInput value={form.holding_structure} onChange={(e) => setForm({ ...form, holding_structure: e.target.value })} placeholder="مجموعة قابضة / SPV" /></Field>
        </div>
        <div className="grid grid-cols-2 gap-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="مرجع العناية الواجبة"><TextInput value={form.due_diligence_ref} onChange={(e) => setForm({ ...form, due_diligence_ref: e.target.value })} placeholder="DD-2025-001" /></Field>
