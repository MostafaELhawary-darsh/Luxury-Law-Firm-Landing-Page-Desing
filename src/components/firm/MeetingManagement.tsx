...
import {
  ...
  Monitor, Award, Zap, FileSignature, UserCheck, Hash, PlayCircle,
} from 'lucide-react';
...
type TabId = 'live' | 'rooms' | 'privilege' | 'esign' | 'ai' | 'transcript' | 'mom' | 'automation';

const TABS: { id: TabId; label: string; icon: typeof Video }[] = [
  { id: 'live', label: 'غرفة الاجتماع المباشر', icon: PlayCircle },
...
export default function MeetingManagement({ voiceAdd }: { voiceAdd?: () => PendingAddCommand | null }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'live' && (
        liveMeetingId ? (
          <Suspense fallback={<div className="flex items-center justify-center py-20"><Loader2 size={24} className="text-gold animate-spin" /></div>}>
            <LiveMeetingRoom
              meeting={meetings.find((m) => m.id === liveMeetingId)!}
              participants={participants}
              onLeave={() => { setLiveMeetingId(null); setActiveTab('rooms'); }}
            />
          </Suspense>
        ) : (
          <LiveRoomSelector meetings={meetings} onJoin={(id) => { setLiveMeetingId(id); fetchMeetingDetails(id); }} onSelect={fetchMeetingDetails} selectedId={selectedMeetingId} participants={participants} />
        )
      )}
...
// ===== LIVE ROOM SELECTOR =====
function LiveRoomSelector({ meetings, onJoin, onSelect, selectedId, participants }: {
  meetings: Meeting[]; onJoin: (id: string) => void; onSelect: (id: string) => void; selectedId: string | null; participants: MeetingParticipant[];
}) {
  return (
    <div className="space-y-4">
      <div className="bg-gradient-to-l from-midnight to-midnight-light rounded-xl p-5 text-cream">
        <div className="flex items-center gap-2 mb-2">
          ...
          <p className="font-heading font-bold text-gold text-sm">غرفة الاجتماع المباشر — Live Meeting Room</p>
        </div>
        <p className="font-body text-[11px] text-cream/70 leading-relaxed">
          غرفة فيديو تفاعلية متكاملة: رؤية جميع المشاركين، تحكم كامل بالميكروفون والكاميرا لكل طرف، عرض المستندات بكافة الصيغ، دردشة كتابية لحظية، رفع اليد، إخفاء الوجه، وتثبيت المتحدث الرئيسي.
        </p>
      </div>
      {meetings.length === 0 && <p className="font-body text-sm text-ink/40 text-center py-8">لا توجد اجتماعات بعد — أنشئ غرفة جديدة من تبويب غرف الانعقاد</p>}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {meetings.map((m) => {
          const meetingDate = new Date(m.scheduled_date);
          const isLive = m.status === 'منعقدة';
          const isUpcoming = m.status === 'مجدولة' && meetingDate >= new Date();
          return (
            <div key={m.id} className={`bg-white rounded-xl border shadow-sm p-5 transition-all ${isLive ? 'border-emerald-300 ring-2 ring-emerald-100' : isUpcoming ? 'border-gold/30 hover:border-gold/50' : 'border-gray-200 opacity-75'}`}>
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  <div className={`w-11 h-11 rounded-lg flex items-center justify-center ${isLive ? 'bg-emerald-50' : 'bg-blue-50'}`}>
                    <Video size={20} className={isLive ? 'text-emerald-600' : 'text-blue-600'} />
                  </div>
                  <div>
                    <p className="font-body text-sm font-bold text-midnight">{m.title}</p>
                    <p className="font-body text-[10px] text-ink/40">{m.platform}</p>
                  </div>
                </div>
                {isLive && <span className="flex items-center gap-1 px-2 py-0.5 bg-red-50 rounded text-[9px] font-body font-bold text-red-600"><span className="w-1.5 h-1.5 bg-red-500 rounded-full animate-pulse" /> مباشر</span>}
              </div>
              <div className="space-y-1.5 mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-body text-[10px] text-ink/60">{meetingDate.toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' })}</span>
                </div>
                <div className="flex items-center gap-2">
                  <Users size={12} className="text-ink/30" />
                  <span className="font-body text-[10px] text-ink/60">{(m.participants || []).length} مشارك</span>
                  {m.privilege_mode && <span className="flex items-center gap-1 text-[9px] text-red-600 font-body font-bold"><ShieldOff size={10} /> سرية</span>}
                </div>
              </div>
              <button
                onClick={() => onJoin(m.id)}
                className={`w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg font-body text-sm font-bold transition-colors ${isLive ? 'bg-emerald-600 text-white hover:bg-emerald-700' : 'bg-midnight text-cream hover:bg-gold hover:text-midnight'}`}
              >
                <PlayCircle size={16} /> دخول الغرفة المباشرة
              </button>
            </div>
          );
        })}
