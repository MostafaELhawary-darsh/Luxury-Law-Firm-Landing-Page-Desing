...
import {
  ...
  FileText, Scale, Gavel, Users, Briefcase,
} from 'lucide-react';
import { supabase, formatDate, formatCurrency } from '@/lib/financeUtils';
import type {
  M76InHouseLegalFile, M76AuditLog,
} from '@/lib/firmTypes';
...
const STAGE_CONFIG: Record<string, { label: string; bg: string; text: string }> = {
  ...
  risk_assessed: { label: 'تقييم مخاطر', bg: 'bg-orange-50', text: 'text-orange-700' },
...
const STAGES = ['draft', 'reviewed', 'risk_assessed', 'approved', 'executed', 'terminated'];

const FILE_TYPE_LABELS: Record<string, string> = {
  contract_review: 'مراجعة عقد',
  legal_opinion: 'رأي قانوني',
  external_counsel: 'مستشار خارجي',
  internal_investigation: 'تحقيق إداري',
...
const FILE_TYPE_ICONS: Record<string, typeof Building2> = {
  ...
  legal_opinion: Scale,
  external_counsel: Briefcase,
  internal_investigation: Gavel,
  compliance_audit: BadgeCheck,
};
...
const CLM_STAGE_LABELS: Record<string, string> = {
  draft: 'مسودة',
  negotiation: 'تفاوض',
  review: 'مراجعة',
  signing: 'توقيع',
...
const emptyForm: LegalFileForm = {
  file_number: '', file_title: '', file_type: 'contract_review', stage: 'draft',
  company_name: '', contract_value: '0', currency: 'SAR',
  external_firm: '', external_firm_fees: '0', risk_level: 'low',
  legal_opinion_ref: '', investigation_status: 'none', clm_stage: 'draft',
  description: '',
};

export default function InHouseLegalEngine({ voiceAdd }: { voiceAdd?: () => PendingAddCommand | null }) {
  const [files, setFiles] = useState<M76InHouseLegalFile[]>([]);
  const [attorneys, setAttorneys] = useState<Attorney[]>([]);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<Tab>('files');
  const [selectedFile, setSelectedFile] = useState<M76InHouseLegalFile | null>(null);
  const [auditLogs, setAuditLogs] = useState<M76AuditLog[]>([]);
  const [allAudit, setAllAudit] = useState<M76AuditLog[]>([]);
  const [detailLoading, setDetailLoading] = useState(false);
  const [modalOpen, setModalOpen] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [form, setForm] = useState<LegalFileForm>(emptyForm);
  ...
  const fetchAll = useCallback(async () => {
    ...
    const [fRes, attRes, auditRes] = await Promise.all([
      supabase.from('m76_inhouse_legal_files')
        .select('*, advisor:lf_attorneys(name)')
        .order('created_at', { ascending: false }),
      supabase.from('lf_attorneys').select('*').order('name'),
      supabase.from('m76_audit_logs').select('*').order('created_at', { ascending: false }).limit(50),
    ]);
    if (fRes.error) console.error('m76 fetch error', fRes.error);
    ...
    setFiles((fRes.data as M76InHouseLegalFile[]) || []);
    setAttorneys((attRes.data as Attorney[]) || []);
    setAllAudit((auditRes.data as M76AuditLog[]) || []);
    setLoading(false);
  }, []);
  ...
  const logAudit = async (fileId: string, action: string, detail: string) => {
    ...
    const { error } = await supabase.from('m76_audit_logs').insert({
  ...
  const openEdit = (f: M76InHouseLegalFile) => {
    setForm({
      file_number: f.file_number, file_title: f.file_title,
      file_type: f.file_type, stage: f.stage,
      company_name: f.company_name || '', contract_value: String(f.contract_value || 0),
      currency: f.currency || 'SAR', external_firm: f.external_firm || '',
      external_firm_fees: String(f.external_firm_fees || 0), risk_level: f.risk_level || 'low',
      legal_opinion_ref: f.legal_opinion_ref || '', investigation_status: f.investigation_status || 'none',
      clm_stage: f.clm_stage || 'draft', description: f.description || '',
    });
    setEditingId(f.id);
    setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    const fees = Number(form.external_firm_fees) || 0;
    const payload = {
      ...
      external_firm: form.external_firm.trim() || null,
      external_firm_fees: fees,
      risk_level: form.risk_level,
      legal_opinion_ref: form.legal_opinion_ref.trim() || null,
      investigation_status: form.investigation_status,
      clm_stage: form.clm_stage,
      description: form.description.trim() || null,
    };
    if (editingId) {
      const { error } = await supabase.from('m76_inhouse_legal_files').update(payload).eq('id', editingId);
      if (error) console.error('update error', error);
      await logAudit(editingId, 'file_updated', 'تحديث بيانات الملف القانوني');
    } else {
      const { data, error } = await supabase.from('m76_inhouse_legal_files').insert(payload).select('id');
      if (error) console.error('insert error', error);
      const newId = data?.[0]?.id;
      if (newId) {
        await logAudit(newId, 'file_created', 'إنشاء ملف قانوني — النوع: ' + (FILE_TYPE_LABELS[form.file_type] || form.file_type));
        const needsCase = form.file_type === 'internal_investigation' || form.risk_level === 'critical';
        const needsHr = form.file_type === 'internal_investigation';
        await supabase.from('m76_inhouse_legal_files').update({
          m53_document_id: 'DOC-M76-' + Date.now().toString().slice(-6),
          m54_finance_linked: true,
          m10_case_opened: needsCase,
          m77_hr_linked: needsHr,
          m109_biometric_signed: true,
          m92_notified: true,
          cost_center_id: 'CC-M76-' + Date.now().toString().slice(-6),
        }).eq('id', newId);
        ...
        if (needsHr) await logAudit(newId, 'm77_hr', 'ربط التحقيق الإداري بمحرك الموارد البشرية (M77)');
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    const { error } = await supabase.from('m76_inhouse_legal_files').delete().eq('id', deleteId);
  ...
  const openFileDetail = async (f: M76InHouseLegalFile) => {
    setSelectedFile(f);
    setDetailLoading(true);
    const aRes = await supabase.from('m76_audit_logs').select('*').eq('case_id', f.id).order('created_at', { ascending: true });
    if (aRes.error) console.error('detail audit error', aRes.error);
    setAuditLogs((aRes.data as M76AuditLog[]) || []);
    setDetailLoading(false);
  };

  const advanceStage = async (f: M76InHouseLegalFile) => {
    const idx = STAGES.indexOf(f.stage);
    if (idx < 0 || idx >= STAGES.length - 1) return;
    const next = STAGES[idx + 1];
    const { error } = await supabase.from('m76_inhouse_legal_files').update({ stage: next, status: next === 'terminated' ? 'terminated' : 'active' }).eq('id', f.id);
    ...
    fetchAll();
    setSelectedFile({ ...f, stage: next } as M76InHouseLegalFile);
  };
  ...
  const filteredFiles = files.filter((f) => {
    ...
    if (searchQuery) {
      ...
      if (!f.file_number.toLowerCase().includes(q) && !f.file_title.toLowerCase().includes(q) && !(f.company_name || '').toLowerCase().includes(q) && !(f.external_firm || '').toLowerCase().includes(q)) return false;
    }
    return true;
  });
  ...
  const highRiskCount = files.filter((f) => f.risk_level === 'high' || f.risk_level === 'critical').length;
  const totalContractValue = files.reduce((s, f) => s + (f.contract_value || 0), 0);
  const totalExternalFees = files.reduce((s, f) => s + (f.external_firm_fees || 0), 0);

  const tabs: { id: Tab; label: string; icon: typeof Building2; badge?: number }[] = [
    { id: 'files', label: 'الملفات القانونية', icon: Building2, badge: files.length },
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-heading font-bold text-midnight text-lg">الإدارات القانونية والامتثال المؤسسي للشركات والمؤسسات (M76)</h2>
            <p className="font-body text-[10px] text-ink/40">إدارة دورة حياة العقود والآراء القانونية والتحقيقات الإدارية والمستشارين الخارجيين</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard icon={<Building2 size={14} className="text-midnight" />} label="إجمالي الملفات" value={String(files.length)} valueClass="text-midnight" />
        <StatCard icon={<AlertTriangle size={14} className="text-red-600" />} label="حالات عالية المخاطر" value={String(highRiskCount)} valueClass="text-red-700" />
        ...
        <StatCard icon={<Briefcase size={14} className="text-blue-600" />} label="إجمالي أتعاب المستشارين" value={formatCurrency(totalExternalFees)} valueClass="text-blue-700" />
      </div>
      ...
      <div className="bg-midnight rounded-xl p-4 border border-gold/20">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="font-heading font-bold text-cream text-xs">دورة حياة الملف القانوني — 6 مراحل</span>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-2">
          {[
            ...
            { icon: Users, label: 'الموارد البشرية (M77)', desc: 'التحقيقات الإدارية', color: 'text-green-600' },
        ...
        <div className="space-y-2">
          {filteredFiles.length === 0 ? (
            <div className="text-center py-12 bg-gray-50 rounded-xl border border-gray-200">
              ...
              <p className="font-body text-xs text-ink/30">لا توجد ملفات قانونية مسجلة</p>
            </div>
          ) : (
            filteredFiles.map((f) => {
              const sCfg = STAGE_CONFIG[f.stage] || STAGE_CONFIG.draft;
              const stageIdx = STAGES.indexOf(f.stage);
              const TypeIcon = FILE_TYPE_ICONS[f.file_type] || Building2;
              const rCfg = RISK_LEVEL_CONFIG[f.risk_level || 'low'] || RISK_LEVEL_CONFIG.low;
              return (
                    ...
                    <div className="flex items-start gap-3 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`px-1.5 py-0.5 rounded text-[9px] font-body font-bold ${rCfg.bg} ${rCfg.text}`}>مخاطر: {RISK_LEVEL_LABELS[f.risk_level || 'low'] || f.risk_level}</span>
                        </div>
                        ...
                        <div className="flex items-center gap-2 mt-2 flex-wrap">
                          ...
                          {f.external_firm && <span className="font-body text-[9px] text-ink/40">المستشار الخارجي: {f.external_firm}</span>}
                          {f.contract_value > 0 && <span className="font-body text-[9px] text-gold font-bold">{formatCurrency(f.contract_value)}</span>}
                          {f.external_firm_fees > 0 && <span className="font-body text-[9px] text-blue-600 font-bold">أتعاب: {formatCurrency(f.external_firm_fees)}</span>}
                          {f.investigation_status && f.investigation_status !== 'none' && <span className="font-body text-[9px] text-orange-600 font-bold">تحقيق: {INVESTIGATION_LABELS[f.investigation_status] || f.investigation_status}</span>}
                          {f.clm_stage && <span className="font-body text-[9px] text-purple-600 font-bold">CLM: {CLM_STAGE_LABELS[f.clm_stage] || f.clm_stage}</span>}
              ...
              {allAudit.map((log) => (
                <div key={log.id} className="flex items-start gap-3 px-4 py-3 hover:bg-gray-50 transition-colors">
                  <div className="w-7 h-7 rounded-lg bg-gray-100 flex items-center justify-center flex-shrink-0">
                    {log.action.includes('created') ? <Building2 size={12} className="text-blue-600" />
          ...
          <div className="relative bg-white w-full max-w-lg h-full overflow-y-auto shadow-2xl flex flex-col">
            <div className="sticky top-0 bg-white border-b border-gray-100 px-5 py-4 flex items-center justify-between z-10">
              <div className="flex items-center gap-2">
                ...
                <span className="font-heading font-bold text-midnight text-sm">ملف الإدارة القانونية</span>
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
                ...
                <div className="bg-gold/5 rounded-lg p-3 border border-gold/10">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div><span className="font-body text-[9px] text-ink/40">المستشار الخارجي</span><p className="font-body text-xs font-bold text-midnight">{selectedFile.external_firm || '—'}</p></div>
                    <div><span className="font-body text-[9px] text-ink/40">مرجع الرأي القانوني</span><p className="font-body text-xs font-bold text-midnight">{selectedFile.legal_opinion_ref || '—'}</p></div>
                  </div>
                </div>
                ...
                <div className="bg-blue-50 rounded-lg p-3 border border-blue-100">
                  <Briefcase size={12} className="text-blue-600 mb-1" />
                  <span className="font-body text-[9px] text-ink/40">أتعاب المستشار الخارجي</span>
                  <p className="font-body text-sm font-bold text-blue-700">{formatCurrency(selectedFile.external_firm_fees)}</p>
                </div>

                {/* Risk level */}
                <div className={`rounded-lg p-3 border ${(RISK_LEVEL_CONFIG[selectedFile.risk_level || 'low'] || RISK_LEVEL_CONFIG.low).bg} border-current`}>
                  <div className="flex items-center gap-1.5 mb-1">
                    <AlertTriangle size={12} className={(RISK_LEVEL_CONFIG[selectedFile.risk_level || 'low'] || RISK_LEVEL_CONFIG.low).text} />
                    <span className="font-body text-[10px] font-bold text-midnight">مستوى المخاطر</span>
                  </div>
                  <p className={`font-body text-xs font-bold ${(RISK_LEVEL_CONFIG[selectedFile.risk_level || 'low'] || RISK_LEVEL_CONFIG.low).text}`}>{RISK_LEVEL_LABELS[selectedFile.risk_level || 'low'] || selectedFile.risk_level}</p>
                </div>

                {/* Investigation status */}
                {selectedFile.investigation_status && selectedFile.investigation_status !== 'none' && (
                  <div className="bg-orange-50 rounded-lg p-3 border border-orange-100">
                    <div className="flex items-center gap-1.5 mb-1">
                      <Gavel size={12} className="text-orange-600" />
                      <span className="font-body text-[10px] font-bold text-orange-700">حالة التحقيق الإداري</span>
                    </div>
                    <p className="font-body text-xs font-bold text-midnight">{INVESTIGATION_LABELS[selectedFile.investigation_status] || selectedFile.investigation_status}</p>
                  </div>
                )}

                {/* CLM stage */}
                {selectedFile.clm_stage && (
                  <div className="bg-purple-50 rounded-lg p-3 border border-purple-100">
                    <div className="flex items-center gap-1.5 mb-1">
                      <FileText size={12} className="text-purple-600" />
                      <span className="font-body text-[10px] font-bold text-purple-700">مرحلة إدارة دورة حياة العقد (CLM)</span>
                    </div>
                    <p className="font-body text-xs font-bold text-midnight">{CLM_STAGE_LABELS[selectedFile.clm_stage] || selectedFile.clm_stage}</p>
                  </div>
                )}
      ...
      <EntityModal open={modalOpen} title={editingId ? 'تعديل الملف' : 'ملف قانوني جديد'} onClose={() => setModalOpen(false)} onSubmit={handleSave} loading={saving}>
        <div className="grid grid-cols-2 gap-4">
          <Field label="رقم الملف" required><TextInput value={form.file_number} onChange={(e) => setForm({ ...form, file_number: e.target.value })} placeholder="IL-2025-001" /></Field>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="المستشار الخارجي"><TextInput value={form.external_firm} onChange={(e) => setForm({ ...form, external_firm: e.target.value })} /></Field>
        </div>
        <div className="grid grid-cols-2 gap-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="أتعاب المستشار الخارجي"><TextInput type="number" value={form.external_firm_fees} onChange={(e) => setForm({ ...form, external_firm_fees: e.target.value })} /></Field>
          <Field label="مرجع الرأي القانوني"><TextInput value={form.legal_opinion_ref} onChange={(e) => setForm({ ...form, legal_opinion_ref: e.target.value })} placeholder="LO-2025-001" /></Field>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Field label="المرحلة">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="حالة التحقيق الإداري">
            <Select value={form.investigation_status} onChange={(e) => setForm({ ...form, investigation_status: e.target.value })}>
              {Object.entries(INVESTIGATION_LABELS).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
            </Select>
          </Field>
          <Field label="مرحلة CLM">
            <Select value={form.clm_stage} onChange={(e) => setForm({ ...form, clm_stage: e.target.value })}>
              {Object.entries(CLM_STAGE_LABELS).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
            </Select>
          </Field>
        </div>
