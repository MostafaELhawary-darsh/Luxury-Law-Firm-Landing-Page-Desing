...
import {
  ...
  FileText, Scale, Gavel, Plane, Briefcase, Stamp, Users,
} from 'lucide-react';
import { supabase, formatDate, formatCurrency } from '@/lib/financeUtils';
import type {
  M97ForeignResidencyFile, M97AuditLog,
} from '@/lib/firmTypes';
...
const FILE_TYPE_LABELS: Record<string, string> = {
  ...
  residency: 'إقامة',
  investor_visa: 'إقامة مستثمر',
  consular: 'توثيق قنصلي',
  deportation: 'ترحيل',
  expert_contract: 'عقد خبير أجنبي',
};

const FILE_TYPE_ICONS: Record<string, typeof Building2> = {
  work_permit: BadgeCheck,
  residency: FileText,
  investor_visa: Briefcase,
  consular: Stamp,
  deportation: AlertTriangle,
  expert_contract: Users,
};

const RESIDENCY_TYPE_LABELS: Record<string, string> = {
  tourist: 'سياحة',
  investor: 'استثمار',
  work: 'عمل',
  study: 'دراسة',
...
interface ForeignResidencyFileForm {
  file_number: string;
  file_title: string;
  file_type: string;
  stage: string;
...
const emptyForm: ForeignResidencyFileForm = {
  file_number: '', file_title: '', file_type: 'work_permit', stage: 'draft',
  foreign_national_name: '', nationality: '', passport_number: '',
  residency_type: 'work', permit_number: '', sponsor_name: '', employer_name: '',
  contract_value: '0', currency: 'SAR', tax_deduction: '0',
  social_insurance: false, consular_ref: '', deportation_flagged: false,
  renewal_deadline: '',
  description: '',
};

export default function ForeignResidencyEngine({ voiceAdd }: { voiceAdd?: () => PendingAddCommand | null }) {
  const [files, setFiles] = useState<M97ForeignResidencyFile[]>([]);
  const [attorneys, setAttorneys] = useState<Attorney[]>([]);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<Tab>('files');
  const [selectedFile, setSelectedFile] = useState<M97ForeignResidencyFile | null>(null);
  const [auditLogs, setAuditLogs] = useState<M97AuditLog[]>([]);
  const [allAudit, setAllAudit] = useState<M97AuditLog[]>([]);
  const [detailLoading, setDetailLoading] = useState(false);
  const [modalOpen, setModalOpen] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [form, setForm] = useState<ForeignResidencyFileForm>(emptyForm);
  ...
  const fetchAll = useCallback(async () => {
    ...
    const [fRes, attRes, auditRes] = await Promise.all([
      supabase.from('m97_foreign_residency_files')
        .select('*, advisor:lf_attorneys(name)')
        .order('created_at', { ascending: false }),
      supabase.from('lf_attorneys').select('*').order('name'),
      supabase.from('m97_audit_logs').select('*').order('created_at', { ascending: false }).limit(50),
    ]);
    if (fRes.error) console.error('m97 fetch error', fRes.error);
    ...
    setFiles((fRes.data as M97ForeignResidencyFile[]) || []);
    setAttorneys((attRes.data as Attorney[]) || []);
    setAllAudit((auditRes.data as M97AuditLog[]) || []);
    setLoading(false);
  }, []);
  ...
  const logAudit = async (fileId: string, action: string, detail: string) => {
    ...
    const { error } = await supabase.from('m97_audit_logs').insert({
  ...
  const openEdit = (f: M97ForeignResidencyFile) => {
    setForm({
      ...
      tax_deduction: String(f.tax_deduction || 0),
      social_insurance: !!f.social_insurance,
      consular_ref: f.consular_ref || '',
      deportation_flagged: !!f.deportation_flagged,
      renewal_deadline: f.renewal_deadline || '',
  ...
  const handleSave = async () => {
    ...
    const taxDeduction = Number(form.tax_deduction) || 0;
    const payload = {
      ...
      deportation_flagged: form.deportation_flagged,
      renewal_deadline: form.renewal_deadline || null,
      description: form.description.trim() || null,
    };
    if (editingId) {
      const { error } = await supabase.from('m97_foreign_residency_files').update(payload).eq('id', editingId);
      if (error) console.error('update error', error);
      await logAudit(editingId, 'file_updated', 'تحديث بيانات ملف شؤون الأجانب والإقامات');
    } else {
      const { data, error } = await supabase.from('m97_foreign_residency_files').insert(payload).select('id');
      if (error) console.error('insert error', error);
      const newId = data?.[0]?.id;
      if (newId) {
        await logAudit(newId, 'file_created', 'إنشاء ملف أجانب — النوع: ' + (FILE_TYPE_LABELS[form.file_type] || form.file_type));
        const needsHr = form.file_type === 'work_permit' || form.file_type === 'expert_contract';
        const needsCase = form.file_type === 'deportation' || form.deportation_flagged;
        await supabase.from('m97_foreign_residency_files').update({
          m53_document_id: 'DOC-M97-' + Date.now().toString().slice(-6),
          m54_finance_linked: true,
          m77_hr_linked: needsHr,
          m10_case_opened: needsCase,
          m109_biometric_signed: true,
          m92_notified: true,
          cost_center_id: 'CC-M97-' + Date.now().toString().slice(-6),
        }).eq('id', newId);
        ...
        if (needsHr) await logAudit(newId, 'm77_hr', 'ربط ملف العمل أو الخبير بمحرك الموارد البشرية (M77)');
        if (needsCase) await logAudit(newId, 'm10_case', 'فتح القضية في المحرك الموحد (M10)');
        await logAudit(newId, 'm109_biometric', 'التحقق البيومتري من هوية الأجنبي (M109)');
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    const { error } = await supabase.from('m97_foreign_residency_files').delete().eq('id', deleteId);
  ...
  const openFileDetail = async (f: M97ForeignResidencyFile) => {
    setSelectedFile(f);
    setDetailLoading(true);
    const aRes = await supabase.from('m97_audit_logs').select('*').eq('case_id', f.id).order('created_at', { ascending: true });
    if (aRes.error) console.error('detail audit error', aRes.error);
    setAuditLogs((aRes.data as M97AuditLog[]) || []);
    setDetailLoading(false);
  };

  const advanceStage = async (f: M97ForeignResidencyFile) => {
    const idx = STAGES.indexOf(f.stage);
    if (idx < 0 || idx >= STAGES.length - 1) return;
    const next = STAGES[idx + 1];
    const { error } = await supabase.from('m97_foreign_residency_files').update({ stage: next, status: next === 'terminated' ? 'terminated' : 'active' }).eq('id', f.id);
    ...
    fetchAll();
    setSelectedFile({ ...f, stage: next } as M97ForeignResidencyFile);
  };
  ...
  const filteredFiles = files.filter((f) => {
    ...
    if (searchQuery) {
      ...
      if (!f.file_number.toLowerCase().includes(q) && !f.file_title.toLowerCase().includes(q) && !(f.foreign_national_name || '').toLowerCase().includes(q) && !(f.nationality || '').toLowerCase().includes(q) && !(f.passport_number || '').toLowerCase().includes(q) && !(f.permit_number || '').toLowerCase().includes(q)) return false;
    }
    return true;
  });
  ...
  const workPermitsCount = files.filter((f) => f.file_type === 'work_permit').length;
  const investorVisasCount = files.filter((f) => f.file_type === 'investor_visa').length;
  const deportationCount = files.filter((f) => f.deportation_flagged).length;

  const tabs: { id: Tab; label: string; icon: typeof Building2; badge?: number }[] = [
    { id: 'files', label: 'ملفات الأجانب والإقامات', icon: Plane, badge: files.length },
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-heading font-bold text-midnight text-lg">شؤون الأجانب المقيمين والعاملين وتراخيص العمل والهجرة الدولية (M97)</h2>
            <p className="font-body text-[10px] text-ink/40">إدارة تصاريح العمل والإقامات والتوفيقات القنصلية وعقود الخبراء الأجانب</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <StatCard icon={<BadgeCheck size={14} className="text-blue-600" />} label="تصاريح العمل" value={String(workPermitsCount)} valueClass="text-blue-700" />
        <StatCard icon={<Briefcase size={14} className="text-gold" />} label="إقامات المستثمرين" value={String(investorVisasCount)} valueClass="text-gold" />
        <StatCard icon={<AlertTriangle size={14} className="text-red-600" />} label="حالات ترحيل" value={String(deportationCount)} valueClass="text-red-700" />
      </div>
      ...
      <div className="bg-midnight rounded-xl p-4 border border-gold/20">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="font-heading font-bold text-cream text-xs">دورة حياة ملف الأجانب والإقامات — 6 مراحل</span>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
          {[
            ...
            { icon: DollarSign, label: 'المحرك المالي (M54)', desc: 'رسوم واستقطاعات', color: 'text-gold' },
            { icon: Users, label: 'الموارد البشرية (M77)', desc: 'ملفات الخبراء', color: 'text-green-600' },
            { icon: Scale, label: 'نواة القضية (M10)', desc: 'نزاعات عقود العمل', color: 'text-blue-600' },
            { icon: BadgeCheck, label: 'البيومتري (M109)', desc: 'تحقق الهوية', color: 'text-green-600' },
          ].map((item, i) => {
      ...
      {activeTab === 'files' && (
        <div className="flex items-center gap-2">
          ...
          <div className="relative flex-1 max-w-xs">
            ...
            <TextInput value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="بحث برقم الملف أو الاسم أو الجنسية..." className="!py-1.5 !text-xs pr-9" />
          </div>
        </div>
      )}
        ...
        <div className="space-y-2">
          {filteredFiles.length === 0 ? (
            <div className="text-center py-12 bg-gray-50 rounded-xl border border-gray-200">
              ...
              <p className="font-body text-xs text-ink/30">لا توجد ملفات أجانب مسجلة</p>
            </div>
          ) : (
            filteredFiles.map((f) => {
                    ...
                    <div className="flex items-start gap-3 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {f.residency_type && (
                            <span className="px-1.5 py-0.5 rounded text-[9px] font-body bg-blue-50 text-blue-600">{RESIDENCY_TYPE_LABELS[f.residency_type] || f.residency_type}</span>
                          )}
                          {f.deportation_flagged && (
                            <span className="flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[9px] font-body font-bold bg-red-50 text-red-600">
                              <AlertTriangle size={8} /> ترحيل
                            </span>
                          )}
                          {f.social_insurance && (
                        ...
                        <div className="flex items-center gap-2 mt-2 flex-wrap">
                          {f.foreign_national_name && <span className="font-body text-[9px] text-ink/40">الأجنبي: {f.foreign_national_name}</span>}
                          {f.nationality && <span className="font-body text-[9px] text-ink/40">الجنسية: {f.nationality}</span>}
                          {f.passport_number && <span className="font-body text-[9px] text-ink/40">جواز: {f.passport_number}</span>}
                          {f.permit_number && <span className="font-body text-[9px] text-blue-600 font-bold">تصريح: {f.permit_number}</span>}
                          {f.contract_value > 0 && <span className="font-body text-[9px] text-gold font-bold">قيمة العقد: {formatCurrency(f.contract_value)}</span>}
                          {f.tax_deduction > 0 && <span className="font-body text-[9px] text-amber-600 font-bold">استقطاع: {formatCurrency(f.tax_deduction)}</span>}
                          {f.consular_ref && <span className="font-body text-[9px] text-purple-600 font-bold">قنصلي: {f.consular_ref}</span>}
                          {f.renewal_deadline && <span className="font-body text-[9px] text-orange-600 font-bold">تجديد: {formatDate(f.renewal_deadline)}</span>}
          ...
          <div className="relative bg-white w-full max-w-lg h-full overflow-y-auto shadow-2xl flex flex-col">
            <div className="sticky top-0 bg-white border-b border-gray-100 px-5 py-4 flex items-center justify-between z-10">
              <div className="flex items-center gap-2">
                ...
                <span className="font-heading font-bold text-midnight text-sm">ملف الأجانب والإقامات</span>
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
                ...
                <div className="bg-gold/5 rounded-lg p-3 border border-gold/10">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div><span className="font-body text-[9px] text-ink/40">اسم الأجنبي</span><p className="font-body text-xs font-bold text-midnight">{selectedFile.foreign_national_name || '—'}</p></div>
                    <div><span className="font-body text-[9px] text-ink/40">الجنسية</span><p className="font-body text-xs font-bold text-midnight">{selectedFile.nationality || '—'}</p></div>
                    <div><span className="font-body text-[9px] text-ink/40">رقم جواز السفر</span><p className="font-body text-xs font-bold text-midnight">{selectedFile.passport_number || '—'}</p></div>
                    <div><span className="font-body text-[9px] text-ink/40">نوع الإقامة</span><p className="font-body text-xs font-bold text-midnight">{RESIDENCY_TYPE_LABELS[selectedFile.residency_type || ''] || selectedFile.residency_type || '—'}</p></div>
                  </div>
                </div>
                ...
                <div className="bg-gold/5 rounded-lg p-3 border border-gold/10">
                  ...
                  {selectedFile.currency && <p className="font-body text-[9px] text-ink/40 mt-0.5">العملة: {selectedFile.currency}</p>}
                </div>

                {/* Sponsor & employer card */}
                <div className="bg-blue-50 rounded-lg p-3 border border-blue-100">
                  <div className="flex items-center gap-1.5 mb-2">
                    <Building2 size={12} className="text-blue-600" />
                    <span className="font-body text-[10px] font-bold text-midnight">الكفيل وصاحب العمل</span>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div><span className="font-body text-[9px] text-ink/40">اسم الكفيل</span><p className="font-body text-xs font-bold text-midnight">{selectedFile.sponsor_name || '—'}</p></div>
                    <div><span className="font-body text-[9px] text-ink/40">اسم صاحب العمل</span><p className="font-body text-xs font-bold text-midnight">{selectedFile.employer_name || '—'}</p></div>
                  </div>
                </div>
                ...
                <div className="bg-amber-50 rounded-lg p-3 border border-amber-100">
                  <div className="flex items-center gap-1.5 mb-2">
                    ...
                    <span className="font-body text-[10px] font-bold text-midnight">الاستقطاعات والتأمين</span>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div><span className="font-body text-[9px] text-ink/40">استقطاعات ضريبية</span><p className="font-body text-xs font-bold text-amber-700">{formatCurrency(selectedFile.tax_deduction)}</p></div>
                    <div><span className="font-body text-[9px] text-ink/40">تأمين اجتماعي</span><p className="font-body text-xs font-bold text-midnight">{selectedFile.social_insurance ? 'مشمول' : 'غير مشمول'}</p></div>
                  </div>
                </div>

                {/* Deportation card */}
                <div className={`rounded-lg p-3 border ${selectedFile.deportation_flagged ? 'bg-red-50 border-red-100' : 'bg-gray-100 border-gray-200'}`}>
                  <div className="flex items-center gap-1.5 mb-1">
                    <AlertTriangle size={12} className={selectedFile.deportation_flagged ? 'text-red-600' : 'text-ink/40'} />
                    <span className="font-body text-[10px] font-bold text-midnight">إجراء الترحيل</span>
                  </div>
                  <p className={`font-body text-xs font-bold ${selectedFile.deportation_flagged ? 'text-red-700' : 'text-ink/40'}`}>
                    {selectedFile.deportation_flagged ? 'مُعلَّم بإجراء ترحيل' : 'لا يوجد إجراء ترحيل'}
                  </p>
                </div>

                {/* Renewal deadline card */}
                <div className="bg-orange-50 rounded-lg p-3 border border-orange-100">
                  <div className="flex items-center gap-1.5 mb-1">
                    <Clock size={12} className="text-orange-600" />
                    <span className="font-body text-[10px] font-bold text-midnight">موعد تجديد الإقامة</span>
                  </div>
                  <p className="font-body text-sm font-bold text-orange-700">{selectedFile.renewal_deadline ? formatDate(selectedFile.renewal_deadline) : '—'}</p>
                </div>
      ...
      <EntityModal open={modalOpen} title={editingId ? 'تعديل الملف' : 'ملف أجانب جديد'} onClose={() => setModalOpen(false)} onSubmit={handleSave} loading={saving}>
        <div className="grid grid-cols-2 gap-4">
          <Field label="رقم الملف" required><TextInput value={form.file_number} onChange={(e) => setForm({ ...form, file_number: e.target.value })} placeholder="FR-2025-001" /></Field>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="الجنسية"><TextInput value={form.nationality} onChange={(e) => setForm({ ...form, nationality: e.target.value })} /></Field>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Field label="رقم جواز السفر"><TextInput value={form.passport_number} onChange={(e) => setForm({ ...form, passport_number: e.target.value })} /></Field>
          <Field label="نوع الإقامة">
            <Select value={form.residency_type} onChange={(e) => setForm({ ...form, residency_type: e.target.value })}>
              {Object.entries(RESIDENCY_TYPE_LABELS).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
            </Select>
          </Field>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Field label="رقم التصريح"><TextInput value={form.permit_number} onChange={(e) => setForm({ ...form, permit_number: e.target.value })} /></Field>
          <Field label="اسم الكفيل"><TextInput value={form.sponsor_name} onChange={(e) => setForm({ ...form, sponsor_name: e.target.value })} /></Field>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Field label="اسم صاحب العمل"><TextInput value={form.employer_name} onChange={(e) => setForm({ ...form, employer_name: e.target.value })} /></Field>
          <Field label="مرجع التوثيق القنصلي"><TextInput value={form.consular_ref} onChange={(e) => setForm({ ...form, consular_ref: e.target.value })} /></Field>
        </div>
        <div className="grid grid-cols-2 gap-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="استقطاعات ضريبية"><TextInput type="number" value={form.tax_deduction} onChange={(e) => setForm({ ...form, tax_deduction: e.target.value })} /></Field>
          <Field label="موعد تجديد الإقامة"><TextInput type="date" value={form.renewal_deadline} onChange={(e) => setForm({ ...form, renewal_deadline: e.target.value })} /></Field>
        </div>
        <div className="flex items-center gap-6">
          <Checkbox checked={form.social_insurance} onChange={(v: boolean) => setForm({ ...form, social_insurance: v })} label="تأمين اجتماعي" />
          <Checkbox checked={form.deportation_flagged} onChange={(v: boolean) => setForm({ ...form, deportation_flagged: v })} label="إجراء ترحيل" />
        </div>
        <Field label="المرحلة">
