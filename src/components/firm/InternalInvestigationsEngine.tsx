...
import {
  ...
  FileText, Gavel, Users, Calendar, UserCheck, UserX, Scale, Video,
} from 'lucide-react';
...
export default function InternalInvestigationsEngine({ voiceAdd }: { voiceAdd?: () => PendingAddCommand | null }) {
  ...
  const tabs: { id: Tab; label: string; icon: typeof SearchIcon; badge?: number }[] = [
    ...
    { id: 'live', label: 'جلسة التحقيق المباشرة', icon: Video },
  ...
  return (
    <div className="space-y-5">
      ...
      {activeTab === 'live' && (
        <LiveSessionLauncher
          sessionLabel="جلسة التحقيق المباشرة"
          contextType="investigation"
          sessions={files.map((f) => ({
            id: f.id,
            title: f.file_number || `ملف تحقيق ${f.id.slice(0, 6)}`,
            date: null,
            status: f.stage || 'active',
            participantNames: [f.respondent_name, f.complainant_name, f.advisor?.name].filter(Boolean) as string[],
            documents: [],
          }))}
          onCreateSession={(ctxId) => {
            ...
            if (!f) return null;
            return {
              title: `تحقيق: ${f.file_number || ctxId.slice(0, 6)}`,
              subtitle: f.file_title || 'جلسة استماع تحقيقية',
              contextType: 'investigation',
              contextId: ctxId,
              participants: [
                { name: f.respondent_name || 'المُستجوب', role: 'مُستجوب' },
                { name: f.complainant_name || f.advisor?.name || 'المحقق', role: 'محقق' },
              ],
              documents: [],
            };
          }}
