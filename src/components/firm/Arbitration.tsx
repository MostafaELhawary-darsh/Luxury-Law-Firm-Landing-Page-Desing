...
import {
  ...
  Fingerprint, Sparkles, Gavel, Activity, Server, Award, Eye, PlayCircle,
} from 'lucide-react';
...
import LiveSessionLauncher from './LiveSessionLauncher';

type Tab = 'cases' | 'arbitrators' | 'virtual_hearings' | 'live' | 'draft_awards' | 'audit';
...
export default function Arbitration({ voiceAdd }: { voiceAdd?: () => PendingAddCommand | null }) {
  ...
  const tabs: { id: Tab; label: string; icon: typeof Scale; badge?: number }[] = [
    ...
    { id: 'live', label: 'جلسة التحكيم المباشرة', icon: PlayCircle },
  ...
  return (
    <div className="space-y-5">
      ...
      {activeTab === 'live' && (
        <LiveSessionLauncher
          sessionLabel="جلسة التحكيم المباشرة"
          contextType="arbitration"
          sessions={cases.map((c) => ({
            id: c.id,
            title: c.case_number || `قضية تحكيم ${c.id.slice(0, 6)}`,
            date: c.filing_date || null,
            status: c.stage || 'active',
            participantNames: [c.case_title, c.arbitration_institution].filter(Boolean) as string[],
            documents: [],
          }))}
          onCreateSession={(ctxId) => {
            const c = cases.find((x) => x.id === ctxId);
            if (!c) return null;
            return {
              title: `تحكيم: ${c.case_number || ctxId.slice(0, 6)}`,
              subtitle: c.case_title || 'جلسة تحكيم',
              contextType: 'arbitration',
              contextId: ctxId,
              participants: [
                { name: c.case_title || 'المدعي', role: 'مدعي' },
                { name: c.arbitration_institution || 'المدعى عليه', role: 'مدعى عليه' },
                { name: c.advisor?.name || 'المحكم', role: 'محكم' },
              ],
              documents: [],
            };
          }}
