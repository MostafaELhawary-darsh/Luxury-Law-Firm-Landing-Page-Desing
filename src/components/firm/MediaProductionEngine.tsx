...
import {
  ...
  Activity, Server, AlertCircle, BadgeCheck, Video, DollarSign,
  FileText, Scale, Gavel, Users, Image, Radio, Film, Music, Tv, MonitorPlay,
} from 'lucide-react';
import { supabase, formatDate, formatCurrency } from '@/lib/financeUtils';
import type {
  M81MediaProductionFile, M81AuditLog,
} from '@/lib/firmTypes';
...
const FILE_TYPE_LABELS: Record<string, string> = {
  ...
  talent_agreement: 'عقد نجم',
  distribution_license: 'ترخيص توزيع',
  copyright_registration: 'تسجيل حقوق ملكية',
  censorship_permit: 'تصريح رقابة',
  piracy_case: 'قضية قرصنة',
};

const FILE_TYPE_ICONS: Record<string, typeof Video> = {
  production_contract: Film,
  talent_agreement: Users,
  distribution_license: Radio,
  copyright_registration: FileText,
  censorship_permit: Shield,
  piracy_case: AlertTriangle,
};
...
const WORK_TYPE_LABELS: Record<string, string> = {
  ...
  documentary: 'وثائقي',
  music_album: 'ألبوم موسيقي',
  web_series: 'مسلسل رقمي',
  commercial: 'إعلان',
};
...
const DISPUTE_LABELS: Record<string, string> = {
  none: 'لا يوجد',
  copyright_infringement: 'انتهاك حقوق ملكية',
  royalty_dispute: 'نزاع نسب',
...
interface FileForm {
  ...
  censorship_license: string;
  dispute_status: string;
  description: string;
}

const emptyForm: FileForm = {
  file_number: '', file_title: '', file_type: 'production_contract', stage: 'draft',
  work_title: '', production_company: '', work_type: 'film', budget_amount: '0',
  currency: 'SAR', royalty_percentage: '0', distribution_platform: 'cinema',
  drm_protected: false, copyright_ref: '', censorship_license: '',
  dispute_status: 'none', description: '',
};

export default function MediaProductionEngine({ voiceAdd }: { voiceAdd?: () => PendingAddCommand | null }) {
  const [files, setFiles] = useState<M81MediaProductionFile[]>([]);
  const [attorneys, setAttorneys] = useState<Attorney[]>([]);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<Tab>('files');
  const [selectedFile, setSelectedFile] = useState<M81MediaProductionFile | null>(null);
  const [auditLogs, setAuditLogs] = useState<M81AuditLog[]>([]);
  const [allAudit, setAllAudit] = useState<M81AuditLog[]>([]);
  ...
  const fetchAll = useCallback(async () => {
    ...
    const [fRes, attRes, auditRes] = await Promise.all([
      supabase.from('m81_media_production_files')
        .select('*, advisor:lf_attorneys(name)')
        .order('created_at', { ascending: false }),
      supabase.from('lf_attorneys').select('*').order('name'),
      supabase.from('m81_audit_logs').select('*').order('created_at', { ascending: false }).limit(50),
    ]);
    if (fRes.error) console.error('m81 fetch error', fRes.error);
    ...
    setFiles((fRes.data as M81MediaProductionFile[]) || []);
    setAttorneys((attRes.data as Attorney[]) || []);
    setAllAudit((auditRes.data as M81AuditLog[]) || []);
    setLoading(false);
  }, []);
  ...
  const logAudit = async (fileId: string, action: string, detail: string) => {
    ...
    const { error } = await supabase.from('m81_audit_logs').insert({
  ...
  const openEdit = (f: M81MediaProductionFile) => {
    setForm({
      file_number: f.file_number, file_title: f.file_title,
      file_type: f.file_type, stage: f.stage,
      work_title: f.work_title ?? '', production_company: f.production_company ?? '',
      work_type: f.work_type ?? '', budget_amount: String(f.budget_amount || 0),
      currency: f.currency || 'SAR', royalty_percentage: String(f.royalty_percentage || 0),
      distribution_platform: f.distribution_platform ?? '', drm_protected: f.drm_protected || false,
      copyright_ref: f.copyright_ref || '', censorship_license: f.censorship_license || '',
  ...
  const handleSave = async () => {
    ...
    const royalty = Number(form.royalty_percentage) || 0;
    const payload = {
      ...
      distribution_platform: form.distribution_platform,
      drm_protected: form.drm_protected,
      copyright_ref: form.copyright_ref.trim() || null,
      censorship_license: form.censorship_license.trim() || null,
      dispute_status: form.dispute_status,
      description: form.description.trim() || null,
    };
    if (editingId) {
      const { error } = await supabase.from('m81_media_production_files').update(payload).eq('id', editingId);
      if (error) console.error('update error', error);
      await logAudit(editingId, 'file_updated', 'تحديث بيانات ملف الإنتاج الإعلامي');
    } else {
      const { data, error } = await supabase.from('m81_media_production_files').insert(payload).select('id');
      if (error) console.error('insert error', error);
      const newId = data?.[0]?.id;
      if (newId) {
        await logAudit(newId, 'file_created', 'إنشاء ملف إنتاج إعلامي — النوع: ' + (FILE_TYPE_LABELS[form.file_type] || form.file_type));
        const needsCase = form.file_type === 'piracy_case' || form.dispute_status !== 'none';
        const needsIP = form.file_type === 'copyright_registration' || form.file_type === 'piracy_case' || form.drm_protected;
        await supabase.from('m81_media_production_files').update({
          m53_document_id: 'DOC-M81-' + Date.now().toString().slice(-6),
          m54_finance_linked: true,
          m10_case_opened: needsCase,
          m80_ip_linked: needsIP,
          m109_biometric_signed: true,
          m92_notified: true,
          cost_center_id: 'CC-M81-' + Date.now().toString().slice(-6),
        }).eq('id', newId);
        ...
        if (needsIP) await logAudit(newId, 'm80_ip', 'ربط حقوق الملكية الفكرية بمحرك الملكية الفكرية (M80)');
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    const { error } = await supabase.from('m81_media_production_files').delete().eq('id', deleteId);
  ...
  const openFileDetail = async (f: M81MediaProductionFile) => {
    setSelectedFile(f);
    setDetailLoading(true);
    const aRes = await supabase.from('m81_audit_logs').select('*').eq('case_id', f.id).order('created_at', { ascending: true });
    if (aRes.error) console.error('detail audit error', aRes.error);
    setAuditLogs((aRes.data as M81AuditLog[]) || []);
    setDetailLoading(false);
  };

  const advanceStage = async (f: M81MediaProductionFile) => {
    const idx = STAGES.indexOf(f.stage);
    if (idx < 0 || idx >= STAGES.length - 1) return;
    const next = STAGES[idx + 1];
    const { error } = await supabase.from('m81_media_production_files').update({ stage: next, status: next === 'terminated' ? 'terminated' : 'active' }).eq('id', f.id);
    ...
    fetchAll();
    setSelectedFile({ ...f, stage: next } as M81MediaProductionFile);
  };
  ...
  const filteredFiles = files.filter((f) => {
    ...
    if (searchQuery) {
      ...
      if (!f.file_number.toLowerCase().includes(q) && !f.file_title.toLowerCase().includes(q) && !(f.work_title || '').toLowerCase().includes(q) && !(f.production_company || '').toLowerCase().includes(q)) return false;
    }
    return true;
  });
  ...
  const totalBudget = files.reduce((s, f) => s + (f.budget_amount || 0), 0);
  const drmCount = files.filter((f) => f.drm_protected).length;

  const tabs: { id: Tab; label: string; icon: typeof Video; badge?: number }[] = [
    { id: 'files', label: 'الملفات', icon: Video, badge: files.length },
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-heading font-bold text-midnight text-lg">شركات الإنتاج الإعلامي والمصنفات الفنية السمعية والبصرية (M81)</h2>
            <p className="font-body text-[10px] text-ink/40">إدارة حقوق الملكية الفكرية وعقود النجوم وتراخيص التوزيع ومكافحة القرصنة وحساب نسب الأرباح</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard icon={<Video size={14} className="text-midnight" />} label="إجمالي الملفات" value={String(files.length)} valueClass="text-midnight" />
        ...
        <StatCard icon={<DollarSign size={14} className="text-gold" />} label="إجمالي الميزانية" value={formatCurrency(totalBudget)} valueClass="text-gold" />
        <StatCard icon={<BadgeCheck size={14} className="text-green-600" />} label="محمي DRM" value={String(drmCount)} valueClass="text-green-700" />
      </div>
      ...
      <div className="bg-midnight rounded-xl p-4 border border-gold/20">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="font-heading font-bold text-cream text-xs">دورة حياة ملف الإنتاج الإعلامي — 5 مراحل</span>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-2">
          {[
            ...
            { icon: DollarSign, label: 'المحرك المالي (M54)', desc: 'ربط الميزانية', color: 'text-gold' },
            { icon: Server, label: 'نواة القضية (M10)', desc: 'فتح القضية', color: 'text-blue-600' },
            { icon: Image, label: 'الملكية الفكرية (M80)', desc: 'حقوق الملكية وDRM', color: 'text-cyan-600' },
      ...
      {activeTab === 'files' && (
        <div className="flex items-center gap-2">
          ...
          <div className="relative flex-1 max-w-xs">
            ...
            <TextInput value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="بحث برقم الملف أو العنوان أو العمل أو الشركة..." className="!py-1.5 !text-xs pr-9" />
          </div>
        </div>
      )}
        ...
        <div className="space-y-2">
          {filteredFiles.length === 0 ? (
            <div className="text-center py-12 bg-gray-50 rounded-xl border border-gray-200">
              <Video size={28} className="text-ink/15 mx-auto mb-2" />
              <p className="font-body text-xs text-ink/30">لا توجد ملفات إنتاج إعلامي مسجلة</p>
            </div>
          ) : (
            filteredFiles.map((f) => {
              const sCfg = STAGE_CONFIG[f.stage] || STAGE_CONFIG.draft;
              const stageIdx = STAGES.indexOf(f.stage);
              const TypeIcon = FILE_TYPE_ICONS[f.file_type] || Video;
              return (
                    ...
                    <div className="flex items-start gap-3 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {f.drm_protected && <span className="flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[9px] font-body bg-green-50 text-green-600"><BadgeCheck size={8} /> DRM</span>}
                          {f.work_type && <span className="px-1.5 py-0.5 rounded text-[9px] font-body bg-purple-50 text-purple-600">{WORK_TYPE_LABELS[f.work_type] || f.work_type}</span>}
                        </div>
                        <p className="font-body text-xs font-bold text-midnight mt-1 leading-snug line-clamp-2">{f.file_title}</p>
                        <div className="flex items-center gap-2 mt-2 flex-wrap">
                          {f.work_title && <span className="font-body text-[9px] text-ink/40">العمل: {f.work_title}</span>}
                          {f.production_company && <span className="font-body text-[9px] text-ink/40">شركة الإنتاج: {f.production_company}</span>}
                          {f.distribution_platform && <span className="font-body text-[9px] text-ink/40">التوزيع: {PLATFORM_LABELS[f.distribution_platform] || f.distribution_platform}</span>}
                          {f.budget_amount > 0 && <span className="font-body text-[9px] text-gold font-bold">{formatCurrency(f.budget_amount)}</span>}
                          {f.royalty_percentage > 0 && <span className="font-body text-[9px] text-ink/40">نسبة: {f.royalty_percentage}%</span>}
                          {f.dispute_status !== 'none' && <span className="font-body text-[9px] text-red-600 font-bold">نزاع: {DISPUTE_LABELS[f.dispute_status ?? ''] || f.dispute_status}</span>}
              ...
              {allAudit.map((log) => (
                <div key={log.id} className="flex items-start gap-3 px-4 py-3 hover:bg-gray-50 transition-colors">
                  <div className="w-7 h-7 rounded-lg bg-gray-100 flex items-center justify-center flex-shrink-0">
                    {log.action.includes('created') ? <Video size={12} className="text-blue-600" />
          ...
          <div className="relative bg-white w-full max-w-lg h-full overflow-y-auto shadow-2xl flex flex-col">
            <div className="sticky top-0 bg-white border-b border-gray-100 px-5 py-4 flex items-center justify-between z-10">
              <div className="flex items-center gap-2">
                ...
                <span className="font-heading font-bold text-midnight text-sm">ملف الإنتاج الإعلامي</span>
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
                ...
                <div className="bg-gold/5 rounded-lg p-3 border border-gold/10">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div><span className="font-body text-[9px] text-ink/40">شركة الإنتاج</span><p className="font-body text-xs font-bold text-midnight">{selectedFile.production_company || '—'}</p></div>
                    <div><span className="font-body text-[9px] text-ink/40">نوع العمل</span><p className="font-body text-xs font-bold text-midnight">{WORK_TYPE_LABELS[selectedFile.work_type ?? ''] || selectedFile.work_type || '—'}</p></div>
                    <div><span className="font-body text-[9px] text-ink/40">منصة التوزيع</span><p className="font-body text-xs font-bold text-midnight">{PLATFORM_LABELS[selectedFile.distribution_platform ?? ''] || selectedFile.distribution_platform || '—'}</p></div>
                    ...
                    <div><span className="font-body text-[9px] text-ink/40">نسبة الأرباح</span><p className="font-body text-xs font-bold text-midnight">{selectedFile.royalty_percentage || 0}%</p></div>
                  </div>
                </div>
                ...
                <div className="bg-gold/5 rounded-lg p-3 border border-gold/10">
                  ...
                  <span className="font-body text-[9px] text-ink/40">الميزانية</span>
                  <p className="font-body text-sm font-bold text-gold">{formatCurrency(selectedFile.budget_amount)}</p>
                </div>

                {/* DRM flag */}
                <div className="flex items-center gap-2 flex-wrap">
                  <span className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-body font-bold ${selectedFile.drm_protected ? 'bg-green-50 text-green-600' : 'bg-gray-100 text-ink/30'}`}><BadgeCheck size={10} /> حماية DRM {selectedFile.drm_protected ? 'مفعّلة' : 'غير مفعّلة'}</span>
                </div>
                ...
                <div className="bg-purple-50 rounded-lg p-3 border border-purple-100">
                  <div className="flex items-center gap-1.5 mb-2">
                    ...
                    <span className="font-body text-[10px] font-bold text-purple-700">المراجع والتراخيص</span>
                  </div>
                  <div className="grid grid-cols-1 gap-1">
                    {selectedFile.copyright_ref && <p className="font-body text-[10px] text-ink/60">مرجع حقوق الملكية: {selectedFile.copyright_ref}</p>}
                    {selectedFile.censorship_license && <p className="font-body text-[10px] text-ink/60">تصريح الرقابة: {selectedFile.censorship_license}</p>}
                    {!selectedFile.copyright_ref && !selectedFile.censorship_license && <p className="font-body text-[10px] text-ink/30">لا توجد مراجع مسجلة</p>}
                  </div>
                </div>
      ...
      <EntityModal open={modalOpen} title={editingId ? 'تعديل الملف' : 'ملف إنتاج إعلامي جديد'} onClose={() => setModalOpen(false)} onSubmit={handleSave} loading={saving}>
        <div className="grid grid-cols-2 gap-4">
          <Field label="رقم الملف" required><TextInput value={form.file_number} onChange={(e) => setForm({ ...form, file_number: e.target.value })} placeholder="MP-2025-001" /></Field>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="شركة الإنتاج"><TextInput value={form.production_company} onChange={(e) => setForm({ ...form, production_company: e.target.value })} /></Field>
        </div>
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="منصة التوزيع">
            <Select value={form.distribution_platform} onChange={(e) => setForm({ ...form, distribution_platform: e.target.value })}>
              {Object.entries(PLATFORM_LABELS).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
            </Select>
          </Field>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Field label="الميزانية"><TextInput type="number" value={form.budget_amount} onChange={(e) => setForm({ ...form, budget_amount: e.target.value })} /></Field>
          <Field label="نسبة الأرباح (%)"><TextInput type="number" value={form.royalty_percentage} onChange={(e) => setForm({ ...form, royalty_percentage: e.target.value })} /></Field>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Field label="المرحلة">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="مرجع حقوق الملكية"><TextInput value={form.copyright_ref} onChange={(e) => setForm({ ...form, copyright_ref: e.target.value })} placeholder="CR-2025-001" /></Field>
          <Field label="تصريح الرقابة"><TextInput value={form.censorship_license} onChange={(e) => setForm({ ...form, censorship_license: e.target.value })} placeholder="CL-2025-001" /></Field>
        </div>
        <Checkbox label="حماية DRM (Digital Rights Management)" checked={form.drm_protected} onChange={(v) => setForm({ ...form, drm_protected: v })} />
