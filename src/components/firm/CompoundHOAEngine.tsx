...
import {
  ...
  Activity, Server, AlertCircle, BadgeCheck, Home, DollarSign,
  FileText, Scale, Gavel, Users, Image, Radio,
} from 'lucide-react';
import { supabase, formatDate, formatCurrency } from '@/lib/financeUtils';
import type {
  M78CompoundFile, M78AuditLog,
} from '@/lib/firmTypes';
...
const STAGE_CONFIG: Record<string, { label: string; bg: string; text: string }> = {
  ...
  notified: { label: 'مُبلَّغ', bg: 'bg-cyan-50', text: 'text-cyan-700' },
  executed: { label: 'منفَّذ', bg: 'bg-green-50', text: 'text-green-700' },
  terminated: { label: 'منتهٍ', bg: 'bg-gray-100', text: 'text-gray-700' },
};

const STAGES = ['draft', 'reviewed', 'notified', 'executed', 'terminated'];

const FILE_TYPE_LABELS: Record<string, string> = {
  maintenance: 'صيانة',
  violation: 'مخالفة',
  meeting: 'جمعية عمومية',
  bylaws: 'لائحة اتحاد',
  fee_collection: 'تحصيل رسوم',
  dispute: 'نزاع',
};

const FILE_TYPE_ICONS: Record<string, typeof Home> = {
  maintenance: Home,
...
const VIOLATION_LABELS: Record<string, string> = {
  ...
  noise: 'مزعج',
  parking: 'موقف سيارات',
  structural: 'تعدي إنشائي',
  nuisance: 'إزعاج',
  unpaid_fees: 'رسوم متأخرة',
};
...
const emptyForm: FileForm = {
  ...
  compound_name: '', unit_number: '', owner_name: '', tenant_name: '',
  maintenance_fund: '0', currency: 'SAR', violation_type: 'none',
  penalty_amount: '0', meeting_ref: '', bylaws_ref: '', description: '',
};

export default function CompoundHOAEngine({ voiceAdd }: { voiceAdd?: () => PendingAddCommand | null }) {
  const [files, setFiles] = useState<M78CompoundFile[]>([]);
  const [attorneys, setAttorneys] = useState<Attorney[]>([]);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<Tab>('files');
  const [selectedFile, setSelectedFile] = useState<M78CompoundFile | null>(null);
  const [auditLogs, setAuditLogs] = useState<M78AuditLog[]>([]);
  const [allAudit, setAllAudit] = useState<M78AuditLog[]>([]);
  ...
  const fetchAll = useCallback(async () => {
    ...
    const [fRes, attRes, auditRes] = await Promise.all([
      supabase.from('m78_compound_files')
        .select('*, advisor:lf_attorneys(name)')
        .order('created_at', { ascending: false }),
      supabase.from('lf_attorneys').select('*').order('name'),
      supabase.from('m78_audit_logs').select('*').order('created_at', { ascending: false }).limit(50),
    ]);
    if (fRes.error) console.error('m78 fetch error', fRes.error);
    ...
    setFiles((fRes.data as M78CompoundFile[]) || []);
    setAttorneys((attRes.data as Attorney[]) || []);
    setAllAudit((auditRes.data as M78AuditLog[]) || []);
    setLoading(false);
  }, []);
  ...
  const logAudit = async (fileId: string, action: string, detail: string) => {
    ...
    const { error } = await supabase.from('m78_audit_logs').insert({
  ...
  const openEdit = (f: M78CompoundFile) => {
    setForm({
      file_number: f.file_number, file_title: f.file_title,
      file_type: f.file_type, stage: f.stage,
      compound_name: f.compound_name ?? '', unit_number: f.unit_number || '',
      owner_name: f.owner_name || '', tenant_name: f.tenant_name || '',
      maintenance_fund: String(f.maintenance_fund || 0), currency: f.currency || 'SAR',
      violation_type: f.violation_type || 'none', penalty_amount: String(f.penalty_amount || 0),
      meeting_ref: f.meeting_ref || '', bylaws_ref: f.bylaws_ref || '', description: f.description || '',
    });
    setEditingId(f.id);
    setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    const fund = Number(form.maintenance_fund) || 0;
    ...
    const payload = {
      ...
      bylaws_ref: form.bylaws_ref.trim() || null,
      description: form.description.trim() || null,
    };
    if (editingId) {
      const { error } = await supabase.from('m78_compound_files').update(payload).eq('id', editingId);
      if (error) console.error('update error', error);
      await logAudit(editingId, 'file_updated', 'تحديث بيانات ملف التجمع السكني');
    } else {
      const { data, error } = await supabase.from('m78_compound_files').insert(payload).select('id');
      if (error) console.error('insert error', error);
      const newId = data?.[0]?.id;
      if (newId) {
        await logAudit(newId, 'file_created', 'إنشاء ملف تجمع سكني — النوع: ' + (FILE_TYPE_LABELS[form.file_type] || form.file_type));
        const opensCase = form.file_type === 'dispute' || form.file_type === 'violation';
        const hrLinked = form.file_type === 'maintenance';
        await supabase.from('m78_compound_files').update({
          m53_document_id: 'DOC-M78-' + Date.now().toString().slice(-6),
          m54_finance_linked: true,
          m10_case_opened: opensCase,
          m77_hr_linked: hrLinked,
          m109_biometric_signed: true,
          m92_notified: true,
          cost_center_id: 'CC-M78-' + Date.now().toString().slice(-6),
        }).eq('id', newId);
        ...
        if (opensCase) await logAudit(newId, 'm10_case', 'فتح القضية في المحرك الموحد (M10)');
        if (hrLinked) await logAudit(newId, 'm77_hr', 'ربط الملف بمحرك الموارد البشرية (M77)');
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    const { error } = await supabase.from('m78_compound_files').delete().eq('id', deleteId);
  ...
  const openFileDetail = async (f: M78CompoundFile) => {
    setSelectedFile(f);
    setDetailLoading(true);
    const aRes = await supabase.from('m78_audit_logs').select('*').eq('case_id', f.id).order('created_at', { ascending: true });
    if (aRes.error) console.error('detail audit error', aRes.error);
    setAuditLogs((aRes.data as M78AuditLog[]) || []);
    setDetailLoading(false);
  };

  const advanceStage = async (f: M78CompoundFile) => {
    const idx = STAGES.indexOf(f.stage);
    if (idx < 0 || idx >= STAGES.length - 1) return;
    const next = STAGES[idx + 1];
    const { error } = await supabase.from('m78_compound_files').update({ stage: next, status: next === 'terminated' ? 'terminated' : 'active' }).eq('id', f.id);
    ...
    fetchAll();
    setSelectedFile({ ...f, stage: next } as M78CompoundFile);
  };
  ...
  const filteredFiles = files.filter((f) => {
    ...
    if (searchQuery) {
      ...
      if (!f.file_number.toLowerCase().includes(q) && !f.file_title.toLowerCase().includes(q) && !(f.compound_name || '').toLowerCase().includes(q) && !(f.owner_name || '').toLowerCase().includes(q) && !(f.tenant_name || '').toLowerCase().includes(q)) return false;
    }
    return true;
  });
  ...
  const activeViolations = files.filter((f) => f.file_type === 'violation' && f.stage !== 'terminated' && (f.violation_type || 'none') !== 'none').length;
  const totalMaintenanceFunds = files.reduce((s, f) => s + (f.maintenance_fund || 0), 0);
  const totalPenalties = files.reduce((s, f) => s + (f.penalty_amount || 0), 0);

  const tabs: { id: Tab; label: string; icon: typeof Home; badge?: number }[] = [
    { id: 'files', label: 'الملفات', icon: Home, badge: files.length },
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-heading font-bold text-midnight text-lg">إدارة التجمعات السكنية واتحادات الشاغلين (M78)</h2>
            <p className="font-body text-[10px] text-ink/40">إدارة الكومباوندات والكيانات السكنية المغلقة واللوائح والموازنات والنزاعات بين الملاك والشاغلين</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard icon={<Home size={14} className="text-midnight" />} label="إجمالي الملفات" value={String(files.length)} valueClass="text-midnight" />
        <StatCard icon={<AlertCircle size={14} className="text-red-600" />} label="مخالفات نشطة" value={String(activeViolations)} valueClass="text-red-700" />
        <StatCard icon={<DollarSign size={14} className="text-gold" />} label="إجمالي صندوق الصيانة" value={formatCurrency(totalMaintenanceFunds)} valueClass="text-gold" />
        <StatCard icon={<Gavel size={14} className="text-red-600" />} label="إجمالي الغرامات" value={formatCurrency(totalPenalties)} valueClass="text-red-700" />
      </div>
      ...
      <div className="bg-midnight rounded-xl p-4 border border-gold/20">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="font-heading font-bold text-cream text-xs">دورة حياة ملف التجمع السكني — 5 مراحل</span>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-2">
          {[
            ...
            { icon: DollarSign, label: 'المحرك المالي (M54)', desc: 'ربط الموازنة', color: 'text-gold' },
            { icon: Server, label: 'نواة القضية (M10)', desc: 'فتح النزاع/المخالفة', color: 'text-blue-600' },
            { icon: Users, label: 'الموارد البشرية (M77)', desc: 'ملفات الصيانة', color: 'text-green-600' },
      ...
      {activeTab === 'files' && (
        <div className="flex items-center gap-2">
          ...
          <div className="relative flex-1 max-w-xs">
            ...
            <TextInput value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="بحث برقم الملف أو العنوان أو الكومباوند أو المالك..." className="!py-1.5 !text-xs pr-9" />
          </div>
        </div>
      )}
        ...
        <div className="space-y-2">
          {filteredFiles.length === 0 ? (
            ...
            filteredFiles.map((f) => {
              ...
              const TypeIcon = FILE_TYPE_ICONS[f.file_type] || Home;
              return (
                    ...
                    <div className="flex items-start gap-3 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {f.violation_type && f.violation_type !== 'none' && <span className="flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[9px] font-body bg-red-50 text-red-600"><AlertCircle size={8} /> {VIOLATION_LABELS[f.violation_type] || f.violation_type}</span>}
                          {f.maintenance_fund > 0 && <span className="flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[9px] font-body bg-green-50 text-green-600"><DollarSign size={8} /> صيانة</span>}
                        </div>
                        <p className="font-body text-xs font-bold text-midnight mt-1 leading-snug line-clamp-2">{f.file_title}</p>
                        <div className="flex items-center gap-2 mt-2 flex-wrap">
                          <span className="font-body text-[9px] text-ink/40">الكومباوند: {f.compound_name}</span>
                          {f.unit_number && <span className="font-body text-[9px] text-ink/40">الوحدة: {f.unit_number}</span>}
                          {f.owner_name && <span className="font-body text-[9px] text-ink/40">المالك: {f.owner_name}</span>}
                          {f.tenant_name && <span className="font-body text-[9px] text-ink/40">الشاغل: {f.tenant_name}</span>}
                          {f.maintenance_fund > 0 && <span className="font-body text-[9px] text-gold font-bold">{formatCurrency(f.maintenance_fund)}</span>}
              ...
              {allAudit.map((log) => (
                <div key={log.id} className="flex items-start gap-3 px-4 py-3 hover:bg-gray-50 transition-colors">
                  <div className="w-7 h-7 rounded-lg bg-gray-100 flex items-center justify-center flex-shrink-0">
                    {log.action.includes('created') ? <Home size={12} className="text-blue-600" />
          ...
          <div className="relative bg-white w-full max-w-lg h-full overflow-y-auto shadow-2xl flex flex-col">
            <div className="sticky top-0 bg-white border-b border-gray-100 px-5 py-4 flex items-center justify-between z-10">
              <div className="flex items-center gap-2">
                ...
                <span className="font-heading font-bold text-midnight text-sm">ملف التجمع السكني</span>
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
                ...
                <div className="bg-gold/5 rounded-lg p-3 border border-gold/10">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div><span className="font-body text-[9px] text-ink/40">الكومباوند</span><p className="font-body text-xs font-bold text-midnight">{selectedFile.compound_name}</p></div>
                    ...
                    <div><span className="font-body text-[9px] text-ink/40">المالك</span><p className="font-body text-xs font-bold text-midnight">{selectedFile.owner_name || '—'}</p></div>
                    <div><span className="font-body text-[9px] text-ink/40">الشاغل</span><p className="font-body text-xs font-bold text-midnight">{selectedFile.tenant_name || '—'}</p></div>
                    ...
                    <div><span className="font-body text-[9px] text-ink/40">نوع المخالفة</span><p className="font-body text-xs font-bold text-midnight">{VIOLATION_LABELS[selectedFile.violation_type || 'none'] || 'لا يوجد'}</p></div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-gold/5 rounded-lg p-3 border border-gold/10">
                    ...
                    <span className="font-body text-[9px] text-ink/40">صندوق الصيانة</span>
                    <p className="font-body text-sm font-bold text-gold">{formatCurrency(selectedFile.maintenance_fund || 0)}</p>
                  </div>
                  <div className="bg-red-50 rounded-lg p-3 border border-red-100">
                    <Gavel size={12} className="text-red-600 mb-1" />
                    <span className="font-body text-[9px] text-ink/40">الغرامة</span>
                    <p className="font-body text-sm font-bold text-red-600">{formatCurrency(selectedFile.penalty_amount || 0)}</p>
                  </div>
                </div>

                {/* References */}
                {(selectedFile.meeting_ref || selectedFile.bylaws_ref) && (
                  <div className="bg-gray-50 rounded-lg p-3 border border-gray-100">
                    ...
                    {selectedFile.meeting_ref && <p className="font-body text-[10px] text-ink/60">مرجع الجمعية العمومية: {selectedFile.meeting_ref}</p>}
                    {selectedFile.bylaws_ref && <p className="font-body text-[10px] text-ink/60">مرجع لائحة الاتحاد: {selectedFile.bylaws_ref}</p>}
                  </div>
                )}
      ...
      <EntityModal open={modalOpen} title={editingId ? 'تعديل الملف' : 'ملف تجمع سكني جديد'} onClose={() => setModalOpen(false)} onSubmit={handleSave} loading={saving}>
        <div className="grid grid-cols-2 gap-4">
          <Field label="رقم الملف" required><TextInput value={form.file_number} onChange={(e) => setForm({ ...form, file_number: e.target.value })} placeholder="M78-2025-001" /></Field>
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="اسم الكومباوند" required><TextInput value={form.compound_name} onChange={(e) => setForm({ ...form, compound_name: e.target.value })} /></Field>
          <Field label="رقم الوحدة"><TextInput value={form.unit_number} onChange={(e) => setForm({ ...form, unit_number: e.target.value })} /></Field>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Field label="اسم المالك"><TextInput value={form.owner_name} onChange={(e) => setForm({ ...form, owner_name: e.target.value })} /></Field>
          <Field label="اسم الشاغل"><TextInput value={form.tenant_name} onChange={(e) => setForm({ ...form, tenant_name: e.target.value })} /></Field>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Field label="صندوق الصيانة"><TextInput type="number" value={form.maintenance_fund} onChange={(e) => setForm({ ...form, maintenance_fund: e.target.value })} /></Field>
          <Field label="العملة">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="مرجع الجمعية العمومية"><TextInput value={form.meeting_ref} onChange={(e) => setForm({ ...form, meeting_ref: e.target.value })} placeholder="MTG-2025-001" /></Field>
          <Field label="مرجع لائحة الاتحاد"><TextInput value={form.bylaws_ref} onChange={(e) => setForm({ ...form, bylaws_ref: e.target.value })} placeholder="BYL-2025-001" /></Field>
        </div>
