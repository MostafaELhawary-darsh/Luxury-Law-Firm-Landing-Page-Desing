...
import {
  Loader2, Plus, Pencil, Trash2, ChevronRight, X, MapPin,
  Shield, CircuitBoard, Zap, CheckCircle2, Clock, ArrowRight, Search,
  FileText, Activity, Server, AlertCircle, BadgeCheck,
  DollarSign, BookOpen, Calendar, Scale, Building2, Landmark,
} from 'lucide-react';
import { supabase, formatDate, formatCurrency } from '@/lib/financeUtils';
import type {
  M42LocalFile, M42AuditLog,
} from '@/lib/firmTypes';
...
const STAGE_CONFIG: Record<string, { label: string; bg: string; text: string }> = {
  ...
  committee: { label: 'اللجنة', bg: 'bg-purple-50', text: 'text-purple-700' },
  approved: { label: 'معتمد', bg: 'bg-green-50', text: 'text-green-700' },
  closed: { label: 'الإغلاق', bg: 'bg-gray-100', text: 'text-gray-700' },
};

const STAGES = ['intake', 'review', 'inspection', 'committee', 'approved', 'closed'];

const FILE_TYPE_LABELS: Record<string, string> = {
  commercial_license: 'ترخيص تجاري',
  building_permit: 'تصريح بناء',
  occupation_permit: 'تصريح إشغال',
  reconciliation: 'مصالحة',
  state_property: 'أملاك دولة',
  demolition: 'إزالة',
};

const FILE_TYPE_ICONS: Record<string, typeof MapPin> = {
  commercial_license: Building2,
  building_permit: Building2,
  occupation_permit: Landmark,
  reconciliation: Scale,
  state_property: Landmark,
...
const RECONCILIATION_STYLES: Record<string, { bg: string; text: string }> = {
...
const emptyForm: LocalFileForm = {
  file_number: '', file_title: '', file_type: 'commercial_license', stage: 'intake',
  applicant_name: '', property_location: '', license_type: '',
  reconciliation_status: 'none', state_property_flag: false, committee_assigned: false,
  fee_amount: '0', description: '',
};

export default function LocalAdministrationEngine({ voiceAdd }: { voiceAdd?: () => PendingAddCommand | null }) {
  const [files, setFiles] = useState<M42LocalFile[]>([]);
  const [attorneys, setAttorneys] = useState<Attorney[]>([]);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<Tab>('files');
  const [selectedFile, setSelectedFile] = useState<M42LocalFile | null>(null);
  const [auditLogs, setAuditLogs] = useState<M42AuditLog[]>([]);
  const [allAudit, setAllAudit] = useState<M42AuditLog[]>([]);
  const [detailLoading, setDetailLoading] = useState(false);
  const [modalOpen, setModalOpen] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [form, setForm] = useState<LocalFileForm>(emptyForm);
  ...
  const fetchAll = useCallback(async () => {
    ...
    const [fileRes, attRes, auditRes] = await Promise.all([
      supabase.from('m42_local_files')
        .select('*, advisor:lf_attorneys(name)')
        .order('created_at', { ascending: false }),
      supabase.from('lf_attorneys').select('*').order('name'),
      supabase.from('m42_audit_logs').select('*').order('created_at', { ascending: false }).limit(50),
    ]);
    if (fileRes.error) console.error('m42 fetch error', fileRes.error);
    ...
    setFiles((fileRes.data as M42LocalFile[]) || []);
    setAttorneys((attRes.data as Attorney[]) || []);
    setAllAudit((auditRes.data as M42AuditLog[]) || []);
    setLoading(false);
  }, []);
  ...
  const logAudit = async (fileId: string, action: string, detail: string) => {
    ...
    const { error } = await supabase.from('m42_audit_logs').insert({
  ...
  const openEdit = (f: M42LocalFile) => {
    setForm({
      file_number: f.file_number, file_title: f.file_title,
      file_type: f.file_type, stage: f.stage,
      applicant_name: f.applicant_name, property_location: f.property_location || '',
      license_type: f.license_type || '', reconciliation_status: f.reconciliation_status,
      state_property_flag: f.state_property_flag || false, committee_assigned: f.committee_assigned || false,
      fee_amount: String(f.fee_amount || 0), description: f.description || '',
    });
    setEditingId(f.id);
    setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      property_location: form.property_location.trim() || null,
      license_type: form.license_type.trim() || null,
      reconciliation_status: form.reconciliation_status,
      state_property_flag: form.state_property_flag,
      committee_assigned: form.committee_assigned,
      fee_amount: Number(form.fee_amount) || 0,
      description: form.description.trim() || null,
    };
    if (editingId) {
      const { error } = await supabase.from('m42_local_files').update(payload).eq('id', editingId);
      if (error) console.error('update error', error);
      await logAudit(editingId, 'local_file_updated', 'تحديث بيانات الملف المحلي');
    } else {
      const { data, error } = await supabase.from('m42_local_files').insert(payload).select('id');
      if (error) console.error('insert error', error);
      const newId = data?.[0]?.id;
      if (newId) {
        await logAudit(newId, 'local_file_created', 'إنشاء ملف محلي — نوع: ' + (FILE_TYPE_LABELS[form.file_type] || form.file_type));
        await supabase.from('m42_local_files').update({
          ...
          m92_notified: true,
          cost_center_id: 'CC-M42-' + Date.now().toString().slice(-6),
        }).eq('id', newId);
        await logAudit(newId, 'm54_finance', 'ربط الملف بالمحرك المالي (M54)');
        await logAudit(newId, 'm83_property', 'ربط الملف بمحرك تقييم العقارات (M83)');
        ...
        await logAudit(newId, 'm109_biometric', 'التوقيع البيومتري للمستشار (M109)');
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    const { error } = await supabase.from('m42_local_files').delete().eq('id', deleteId);
  ...
  const openFileDetail = async (f: M42LocalFile) => {
    setSelectedFile(f);
    setDetailLoading(true);
    const aRes = await supabase.from('m42_audit_logs').select('*').eq('case_id', f.id).order('created_at', { ascending: true });
    if (aRes.error) console.error('detail audit error', aRes.error);
    setAuditLogs((aRes.data as M42AuditLog[]) || []);
    setDetailLoading(false);
  };

  const advanceStage = async (f: M42LocalFile) => {
    const idx = STAGES.indexOf(f.stage);
    if (idx < 0 || idx >= STAGES.length - 1) return;
    const next = STAGES[idx + 1];
    const { error } = await supabase.from('m42_local_files').update({ stage: next, status: next === 'closed' ? 'closed' : 'active' }).eq('id', f.id);
    ...
    fetchAll();
    setSelectedFile({ ...f, stage: next } as M42LocalFile);
  };
  ...
  const filteredFiles = files.filter((f) => {
    ...
    if (searchQuery) {
      ...
      if (!f.file_number.toLowerCase().includes(q) && !f.file_title.toLowerCase().includes(q) &&
          ...
          !(f.property_location || '').toLowerCase().includes(q)) return false;
    }
    return true;
  });
  ...
  const totalFees = files.reduce((s, f) => s + (f.fee_amount || 0), 0);
  const statePropertyCount = files.filter((f) => f.state_property_flag).length;

  const tabs: { id: Tab; label: string; icon: typeof MapPin; badge?: number }[] = [
    { id: 'files', label: 'الملفات المحلية', icon: MapPin, badge: files.length },
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-heading font-bold text-midnight text-lg">الإدارة المحلية والمحلات والترخيص والتنظيم العمراني (M62)</h2>
            <p className="font-body text-[10px] text-ink/40">إدارة التراخيص التجارية وتصاريح البناء والإشغالات والمصالحات والأملاك</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard icon={<MapPin size={14} className="text-midnight" />} label="إجمالي الملفات" value={String(files.length)} valueClass="text-midnight" />
        ...
        <StatCard icon={<DollarSign size={14} className="text-gold" />} label="إجمالي الرسوم" value={formatCurrency(totalFees)} valueClass="text-gold" />
        <StatCard icon={<Landmark size={14} className="text-purple-600" />} label="أملاك دولة" value={String(statePropertyCount)} valueClass="text-purple-700" />
      </div>
      ...
      <div className="bg-midnight rounded-xl p-4 border border-gold/20">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="font-heading font-bold text-cream text-xs">دورة حياة الملف المحلي — 6 مراحل</span>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-2">
          {[
            ...
            { icon: Activity, label: 'إنترنت الأشياء (M107)', desc: 'استشعار', color: 'text-cyan-600' },
            { icon: BookOpen, label: 'محرك الزكاة (M46)', desc: 'التحقق الامتثال', color: 'text-amber-600' },
            { icon: BadgeCheck, label: 'البيومتري (M109)', desc: 'توقيع المستشار', color: 'text-green-600' },
      ...
      {activeTab === 'files' && (
        <div className="flex items-center gap-2">
          ...
          <div className="relative flex-1 max-w-xs">
            ...
            <TextInput value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="بحث برقم أو عنوان الملف أو المتقدم..." className="!py-1.5 !text-xs pr-9" />
          </div>
        </div>
      )}
        ...
        <div className="space-y-2">
          {filteredFiles.length === 0 ? (
            <div className="text-center py-12 bg-gray-50 rounded-xl border border-gray-200">
              <MapPin size={28} className="text-ink/15 mx-auto mb-2" />
              <p className="font-body text-xs text-ink/30">لا توجد ملفات محلية مسجلة</p>
            </div>
          ) : (
            filteredFiles.map((f) => {
              const sCfg = STAGE_CONFIG[f.stage] || STAGE_CONFIG.intake;
              const stageIdx = STAGES.indexOf(f.stage);
              const TypeIcon = FILE_TYPE_ICONS[f.file_type] || MapPin;
              const recCfg = RECONCILIATION_STYLES[f.reconciliation_status] || RECONCILIATION_STYLES.none;
              return (
                    ...
                    <div className="flex items-start gap-3 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`px-1.5 py-0.5 rounded text-[9px] font-body font-bold ${recCfg.bg} ${recCfg.text}`}>مصالحة: {RECONCILIATION_LABELS[f.reconciliation_status] || f.reconciliation_status}</span>
                          {f.state_property_flag && <span className="flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[9px] font-body bg-purple-50 text-purple-600 font-bold"><Landmark size={8} /> أملاك دولة</span>}
                          {f.committee_assigned && <span className="flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[9px] font-body bg-cyan-50 text-cyan-600 font-bold"><Scale size={8} /> لجنة</span>}
                        </div>
                        <p className="font-body text-xs font-bold text-midnight mt-1 leading-snug line-clamp-2">{f.file_title}</p>
                        <div className="flex items-center gap-2 mt-2 flex-wrap">
                          <span className="font-body text-[9px] text-ink/40">المتقدم: {f.applicant_name}</span>
                          {f.property_location && <span className="font-body text-[9px] text-ink/40">الموقع: {f.property_location}</span>}
                          {f.license_type && <span className="font-body text-[9px] text-ink/40">نوع الترخيص: {f.license_type}</span>}
                          {f.fee_amount > 0 && <span className="font-body text-[9px] text-gold font-bold"><DollarSign size={9} className="inline" />{formatCurrency(f.fee_amount)}</span>}
              ...
              {allAudit.map((log) => (
                <div key={log.id} className="flex items-start gap-3 px-4 py-3 hover:bg-gray-50 transition-colors">
                  <div className="w-7 h-7 rounded-lg bg-gray-100 flex items-center justify-center flex-shrink-0">
                    {log.action.includes('created') ? <MapPin size={12} className="text-blue-600" />
          ...
          <div className="relative bg-white w-full max-w-lg h-full overflow-y-auto shadow-2xl flex flex-col">
            <div className="sticky top-0 bg-white border-b border-gray-100 px-5 py-4 flex items-center justify-between z-10">
              <div className="flex items-center gap-2">
                ...
                <span className="font-heading font-bold text-midnight text-sm">ملف محلي</span>
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
                ...
                <div className="bg-gold/5 rounded-lg p-3 border border-gold/10">
                  <div className="flex items-center gap-1.5 mb-2">
                    ...
                    <span className="font-body text-[10px] font-bold text-midnight">بيانات الملف المحلي</span>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div><span className="font-body text-[9px] text-ink/40">المتقدم</span><p className="font-body text-xs font-bold text-midnight">{selectedFile.applicant_name}</p></div>
                    ...
                    {selectedFile.property_location && <div className="col-span-2"><span className="font-body text-[9px] text-ink/40">موقع العقار</span><p className="font-body text-xs font-bold text-midnight">{selectedFile.property_location}</p></div>}
                    <div><span className="font-body text-[9px] text-ink/40">حالة المصالحة</span><p className="font-body text-xs font-bold text-midnight">{RECONCILIATION_LABELS[selectedFile.reconciliation_status] || selectedFile.reconciliation_status}</p></div>
                    <div><span className="font-body text-[9px] text-ink/40">الرسوم</span><p className="font-body text-xs font-bold text-gold">{formatCurrency(selectedFile.fee_amount)}</p></div>
                  </div>
                </div>

                {/* Flags */}
                <div className="flex items-center gap-2 flex-wrap">
                  <span className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-body font-bold ${selectedFile.state_property_flag ? 'bg-purple-50 text-purple-600' : 'bg-gray-100 text-ink/30'}`}><Landmark size={10} /> أملاك دولة {selectedFile.state_property_flag ? 'نعم' : 'لا'}</span>
                  <span className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-body font-bold ${selectedFile.committee_assigned ? 'bg-cyan-50 text-cyan-600' : 'bg-gray-100 text-ink/30'}`}><Scale size={10} /> لجنة {selectedFile.committee_assigned ? 'معيَّنة' : 'غير معيَّنة'}</span>
                </div>
      ...
      <EntityModal open={modalOpen} title={editingId ? 'تعديل الملف المحلي' : 'ملف محلي جديد'} onClose={() => setModalOpen(false)} onSubmit={handleSave} loading={saving}>
        <div className="grid grid-cols-2 gap-4">
          <Field label="رقم الملف" required><TextInput value={form.file_number} onChange={(e) => setForm({ ...form, file_number: e.target.value })} placeholder="LOC-2025-001" /></Field>
        ...
        <Field label="موقع العقار"><TextInput value={form.property_location} onChange={(e) => setForm({ ...form, property_location: e.target.value })} /></Field>
        <div className="grid grid-cols-2 gap-4">
          <Field label="حالة المصالحة">
            <Select value={form.reconciliation_status} onChange={(e) => setForm({ ...form, reconciliation_status: e.target.value })}>
              {Object.entries(RECONCILIATION_LABELS).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
            </Select>
          </Field>
          <Field label="المرحلة">
        ...
        <Field label="مبلغ الرسوم"><TextInput type="number" value={form.fee_amount} onChange={(e) => setForm({ ...form, fee_amount: e.target.value })} /></Field>
        <Checkbox label="أملاك دولة (State Property Flag)" checked={form.state_property_flag} onChange={(v) => setForm({ ...form, state_property_flag: v })} />
        <Checkbox label="لجنة معيَّنة (Committee Assigned)" checked={form.committee_assigned} onChange={(v) => setForm({ ...form, committee_assigned: v })} />
