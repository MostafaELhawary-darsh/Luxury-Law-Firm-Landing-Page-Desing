...
export default function LibraryDashboard() {
  ...
  const [stats, setStats] = useState({ total: 0, byType: {} as Record<string, number>, byStatus: {} as Record<string, number>, totalSize: 0 });
  ...
  const fetchDocuments = useCallback(async () => {
    ...
    if (error) {
      ...
      setStats({ total: (data as LegalDocument[])?.length || 0, byType, byStatus, totalSize });
    }
    setLoading(false);
  }, []);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard icon={FileType} label="أنواع المستندات" value={Object.keys(stats.byType).length} color="text-gold" bg="bg-amber-50" />
        <StatCard icon={CheckCircle2} label="مستندات مكتملة" value={stats.byStatus['uploaded'] || 0 + (stats.byStatus['compliant'] || 0)} color="text-green-600" bg="bg-green-50" />
        <StatCard icon={HardDrive} label="الحجم الإجمالي" value={formatFileSize(stats.totalSize)} color="text-purple-600" bg="bg-purple-50" />
      </div>
